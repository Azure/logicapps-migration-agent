<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/block-connections-connectors -->
...
If your organization doesn't permit connecting to restricted or unapproved resources using their [managed connectors](https://learn.microsoft.com/en-us/azure/connectors/managed) in Azure Logic Apps, you can block the capability to create and use those connections in logic app workflows. With Azure Policy, you can define and enforce [policies](https://learn.microsoft.com/en-us/azure/governance/policy/overview.md#policy-definition) that prevent creating or using connections for connectors that you want to block. For example, for security reasons, you might want to block connections to specific social media platforms or other services and systems.

This guide shows you how to set up a policy that blocks specific connections by using the Azure portal. You can also create policy definitions in other ways. For example, you can use the Azure REST API, Azure PowerShell, Azure CLI, or Azure Resource Manager templates. For more information, see [Create and manage policies to enforce compliance](https://learn.microsoft.com/en-us/azure/governance/policy/tutorials/create-and-manage).
...
1. Review the [list of all Azure Logic Apps managed connectors](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors).
...
1. In the [Azure portal](https://portal.azure.com) search box, enter *policy*, and select **Policy**.
   ...
   ![Screenshot shows the Azure portal search box with policy entered and Policy highlighted.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/find-select-azure-policy.png)

1. On the **Policy** menu, under **Authoring**, select **Definitions**. On the **Definitions** toolbar, select **Policy definition**.

   ![Screenshot shows the Definitions page with Policy definition highlighted.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/add-new-policy-definition.png)
...
1. On the **Policy definition** page, provide the information for your policy definition, based on the properties in the table that follow the image:
   ...
   ![Screenshot shows the policy definition values for blocking Gmail.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/policy-definition-create-connection.png)
...
1. Under **Policy rule**, the JSON edit box is prepopulated with a policy definition template. Replace this template with your [policy definition](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure) based on the properties described in the following table and using this syntax:
   ...
   |---------|-------|-------------|
   | `mode` | `All` | The mode that determines the resource types that the policy evaluates. <br><br>This scenario sets `mode` to `All`, which applies the policy to Azure resource groups, subscriptions, and all resource types. <br><br>For more information, see [Policy definition structure - mode](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure.md#mode). |
   | `if` | `{condition-to-evaluate}` | The condition that determines when to enforce the policy rule <br><br>In this scenario, the `{condition-to-evaluate}` determines whether the `api.id` value in `Microsoft.Web/connections/api.id` matches on `*managedApis/{connector-name}`, which specifies a wildcard (*) value. <br><br>For more information, see [Policy definition structure - Policy rule](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-policy-rule). |
   | `field` | `Microsoft.Web/connections/api.id` | The `field` value to compare against the condition <br><br>In this scenario, the `field` uses the [*alias*](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-alias), `Microsoft.Web/connections/api.id`, to access the value in the connector property, `api.id`. |
   ...
   | `then` | `{effect-to-apply}` | The effect to apply when the `if` condition is met <br><br>In this scenario, the `{effect-to-apply}` is to block and fail a request or operation that doesn't comply with the policy. <br><br>For more information, see [Policy definition structure - Policy rule](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-policy-rule). |
   | `effect` | `deny` | The `effect` is to block the request, which is to create the specified connection <br><br>For more information, see [Understand Azure Policy effects - Deny](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/effect-deny). |
   ...
   ![Screenshot shows the Policy rule box with a policy rule example.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/policy-definition-create-connection-rule.png)
...
- [Azure Policy built-in policy definitions for Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/policy-reference)
...
1. Under **Policy definition**, provide the information for your policy definition, based on the properties in the table that follows the image.
   ...
   ![Screenshot shows policy definition values for saving Gmail connections.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/policy-definition-use-connection.png)
...
1. Under **Policy Rule**, the JSON edit box is prepopulated with a policy definition template. Replace this template with your [policy definition](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure) based on the properties described in the following table and using this syntax:
   ...
   |---------|-------|-------------|
   | `mode` | `All` | The mode that determines the resource types that the policy evaluates. <br><br>This scenario sets `mode` to `All`, which applies the policy to Azure resource groups, subscriptions, and all resource types. <br><br>For more information, see [Policy definition structure - mode](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure.md#mode). |
   | `if` | `{condition-to-evaluate}` | The condition that determines when to enforce the policy rule <br><br>In this scenario, the `{condition-to-evaluate}` determines whether the string output from `[string(field('Microsoft.Logic/workflows/parameters'))]`, contains the string, `{connector-name}`. <br><br>For more information, see [Policy definition structure - Policy rule](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-policy-rule). |
   ...
   | `then` | `{effect-to-apply}` | The effect to apply when the `if` condition is met <br><br>In this scenario, the `{effect-to-apply}` is to block and fail a request or operation that doesn't comply with the policy. <br><br>For more information, see [Policy definition structure - Policy rule](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-policy-rule). |
   | `effect` | `deny` | The `effect` is to `deny` or block the request to save a logic app that uses the specified connection <br><br>For more information, see [Understand Azure Policy effects - Deny](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/effect-deny). |
   ...
   ![Screenshot shows a policy definition rule.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/policy-definition-use-connection-rule.png)
...
1. On the **Policy** menu, under **Authoring**, select **Assignments**. On the **Assignments** toolbar, select **Assign policy**.
   ...
   ![Screenshot shows Assignments toolbar with Assign policy highlighted.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/add-new-policy-assignment.png)
...
1. On the **Assign policy** page, under **Basics**, provide this information for the policy assignment:
   ...
   ![Screenshot shows policy assignment values.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/block-connections-connectors/policy-assignment-basics.png)
...
For more information, see [Quickstart: Create a policy assignment to identify noncompliant resources](https://learn.microsoft.com/en-us/azure/governance/policy/assign-policy-portal).
