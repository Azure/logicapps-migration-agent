<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/devops-deployment-single-tenant-azure-logic-apps -->
...
![Conceptual diagram showing separate deployment pipelines for apps and infrastructure.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/devops-deployment-single-tenant/deployment-pipelines-logic-apps.png)

<a name="resource-structure"></a>

### Logic app resource structure

[!INCLUDE [Visual Studio Code - logic app resource structure](https://learn.microsoft.com/en-us/azure/logic-apps/includes/logic-apps-single-tenant-resource-structure)]
...
The Azure Logic Apps ecosystem provides [over 1,000 Microsoft-managed and Azure-hosted connectors](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors) and [built-in operations](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference) as part of a constantly growing collection that you can use in single-tenant Azure Logic Apps. The way that Microsoft maintains managed connectors stays mostly the same in single-tenant Azure Logic Apps as in multi-tenant Azure Logic Apps.
...
For a logic app that is hosted and run in Azure, a [managed identity](https://learn.microsoft.com/en-us/azure/logic-apps/create-managed-service-identity) is the default and recommended authentication type to use for authenticating managed connections that are hosted and run in Azure. In your logic app project's **connections.json** file, the managed connection has an **`authentication`** object that specifies **`ManagedServiceIdentity`** as the authentication type:
...
- [Set up DevOps deployment for single-tenant Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-devops-deployment-single-tenant-azure-logic-apps)
