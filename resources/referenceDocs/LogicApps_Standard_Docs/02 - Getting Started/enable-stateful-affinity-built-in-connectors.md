<!-- Source: https://learn.microsoft.com/en-us/azure/connectors/enable-stateful-affinity-built-in-connectors -->
...
- The Standard logic app resource where you plan to create the workflow that uses the stateful mode-enabled connector operations. If you don't have this resource, [create your Standard logic app resource now](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal).
...
- An Azure virtual network with a subnet to integrate with your logic app. If you don't have these items, see the following documentation:
  ...
  - [Quickstart: Create a virtual network with the Azure portal](https://learn.microsoft.com/en-us/azure/virtual-network/quick-create-portal)
...
If you use network security groups in your virtual network, stateful mode requires that you open [ports 20,000 to 30,000](https://learn.microsoft.com/en-us/azure/app-service/overview-vnet-integration#private-ports).
...
1. To enable virtual network integration for your logic app, and add your logic app to the previously created subnet, follow these steps:
   ...
   1. When you're done, select **Connect**, and return to the **Networking** page.
      ...
      ![Screenshot shows Azure portal, Standard logic app resource, Networking page with selected virtual network and subnet.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/enable-stateful-affinity-built-in-connectors/enable-virtual-network-integration.png)

   For general information about enabling virtual network integration with your app, see [Enable virtual network integration in Azure App Service](https://learn.microsoft.com/en-us/azure/app-service/configure-vnet-integration-enable).
...
1. On the **Configuration** page, select **General settings**.
   ...
   ![Screenshot shows Azure portal, Standard logic app menu, Settings, and Configuration with general settings tab selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/enable-stateful-affinity-built-in-connectors/general-settings.png)

1. Under **Platform settings**, in the **VNet Private Ports** box, enter the ports that you want to use.

   ![Screenshot shows Azure portal, Standard logic app menu, Settings, and Configuration with general settings tab selected, and highlighted property named VNet Private Ports.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/enable-stateful-affinity-built-in-connectors/vnet-private-ports.png)

### Azure Resource Management API

To complete this task with the [Azure Resource Management API - Update By Id](https://learn.microsoft.com/en-us/rest/api/resources/resources/update-by-id), review the following requirements, syntax, and parameter values.
...
- [Set-AzContext](https://learn.microsoft.com/en-us/powershell/module/az.accounts/set-azcontext)
- [Get-AzResource](https://learn.microsoft.com/en-us/powershell/module/az.resources/get-azresource)
- [Set-AzResource](https://learn.microsoft.com/en-us/powershell/module/az.resources/set-azresource)
...
1. On the **Scale out (App Service plan)** page, in the **App Scale out** section, follow these steps:
   ...
   1. Set **Always Ready Instances** to the same number as **Maximum Scale Out Limit** and **Maximum Burst**, which appears in the **Plan Scale out** section, for example:
      ...
      ![Screenshot shows Azure portal, Standard logic app resource, Scale out page, and Always Ready Instances number set to match Maximum Burst and Maximum Scale Out Limit.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/enable-stateful-affinity-built-in-connectors/scale-in-settings.png)

1. When you're done, on the **Scale out** toolbar, select **Save**.

## Related content

- [Connect to Azure Service Bus](https://learn.microsoft.com/en-us/azure/connectors/connectors-create-api-servicebus)
- [Connect to SAP](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/sap)
- [Connect to IBM MQ](https://learn.microsoft.com/en-us/azure/connectors/connectors-create-api-mq)
