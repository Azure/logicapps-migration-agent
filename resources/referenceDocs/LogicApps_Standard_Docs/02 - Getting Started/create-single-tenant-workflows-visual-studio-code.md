<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-visual-studio-code -->
...
Your Standard logic app project can include both [stateful and stateless workflows](single-tenant-overview-compare#stateful-stateless).
...
![Screenshot that shows Visual Studio Code, logic app workspace, project, and workflow.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/visual-studio-code-logic-apps-overview.png)

While this guide's example is cloud-based and only has a few steps, you can create workflows using operations from [1,400+ connectors](https://learn.microsoft.com/en-us/azure/connectors/introduction) that help you integrate a vast range of services, systems, apps, and data across cloud, on premises, and hybrid environments.
...
If you plan to locally create and run workflows using only [built-in operations](https://learn.microsoft.com/en-us/azure/connectors/built-in), which run natively on the Azure Logic Apps (Standard) runtime, you don't need the access and connectivity requirements in this section. However, for the following scenarios, you need to meet these requirements:

- Deploy your logic app to Azure from Visual Studio Code.
- Build a workflow that uses [managed connector operations](https://learn.microsoft.com/en-us/azure/connectors/managed) that run in global Azure.
...
If you don't have an Office 365 account, you can use any other available action that can send messages from your email account, for example, Outlook.com. If you choose a [different email connector](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors), such as Outlook.com, you can still follow the example, and the general overall steps are the same. However, your experience and options might differ in some ways. For example, with the Outlook.com connector, use your personal Microsoft account to sign in.

### Tools

Download and install [**Visual Studio Code**](https://code.visualstudio.com/), which is free.

Download and install the **Azure Logic Apps (Standard) extension for Visual Studio Code** by following these steps:

In Visual Studio Code, on the Activity bar, select **Extensions**. (Keyboard: Press Ctrl+Shift+X)

In the extensions search box, enter **azure logic apps standard**. From the results list, select **Azure Logic Apps (Standard)** **>** **Install**.
...
![Screenshot shows Visual Studio Code with Azure Logic Apps (Standard) extension installed.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/azure-logic-apps-extension-installed.png)

Reload or restart Visual Studio Code to make sure the extension and all dependencies correctly install.

To confirm that the extension and all dependencies correctly installed, see [Check extension installation](#check-extensions).
...
To locally run webhook-based triggers and actions, such as the [built-in **HTTP Webhook** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-webhook), you need to [set up forwarding for the callback URL](#webhook-setup) in Visual Studio Code.

To enable diagnostics logging and tracing with [Application Insights](https://learn.microsoft.com/en-us/azure/azure-monitor/app/app-insights-overview) for your Standard logic app, you can complete this task either during logic app deployment or after deployment. You need to have an Application Insights resource, but you can [create this resource in advance](https://learn.microsoft.com/en-us/azure/azure-monitor/app/create-workspace-resource), during deployment, or after deployment.
...
![Screenshot shows Visual Studio Code settings for Azure Logic Apps (Standard) extension.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/azure-logic-apps-settings.png)

Note

This version is required to use the [Inline Code Operations actions](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-add-run-inline-code).
...
![Screenshot shows Visual Studio Code Activity Bar and selected Azure icon.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/visual-studio-code-azure-icon.png)
...
![Screenshot shows Azure pane with selected link for Azure sign in.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/sign-in-azure-subscription.png)
...
![Screenshot shows Azure pane with subscriptions and selected filter icon.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/filter-subscription-list.png)
...
![Screenshot shows Azure pane, Workspace toolbar, Azure Logic Apps menu, and selected item, Create new logic app workspace.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/create-new-workspace.png)
...
![Screenshot shows Select Folder box and new workspace and project folder with Select button selected.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/select-project-folder.png)
...
![Screenshot shows prompt to provide a workspace name.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/workspace-name.png)
...
![Screenshot shows prompt to select a project template with selected option for logic app.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/project-template.png)
...
![Screenshot shows opened logic app project with prompt to enable Azure-hosted connectors.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/local-workspace-project-created.png)
...
Locally run webhook-based triggers or actions.
[Set up forwarding for webhook callback URLs](#webhook-setup).

Set up stateless workflow run history.
[Enable run history for stateless workflows](#enable-run-history-stateless).

Add artifacts and dependencies, such as maps, schemas, and assemblies.
[Add artifacts and dependencies](#add-artifacts-dependencies).

Work with a NuGet-based (.NET) project
[Convert an extension-based bundle (Node.js) project to a NuGet package-based (.NET) project](#convert-project-nuget). 

**Note**: To convert an extension bundle-based (Node.js) project that you created before assemblies support existed, see also [Migrate NuGet package-based projects to use assemblies in the **lib** folder](#migrate-nuget-assemblies).

Create your own built-in connectors.
1. [Convert an extension bundle-based (Node.js) project to a NuGet package-based (.NET) project ](#convert-project-nuget). 

2. [Enable-built-in-connector-authoring](#enable-built-in-connector-authoring).
...
![Screenshot shows Explorer pane, workflow.json file shortcut menu, and Open Designer selected.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/open-workflow-designer.png)
...
![Screenshot shows designer with blank workflow.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/designer-opened.png)
...
For more information, see [Receive and respond to inbound HTTPs calls to workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres?tabs=standard#add-a-request-trigger).

**Office 365 Outlook**
**Send an email**
Action (managed)
Send an email using an Office 365 Outlook account. To follow the steps in this guide, you need an Office 365 Outlook email account. For more information, see [Connect to Office 365 Outlook from Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/connectors-create-api-office365-outlook). 
...
**Response**
Action (built-in)
Send a reply and return data back to the caller. 

For more information, see [Add a **Response** action](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#tabs=standard#add-a-response-action).

### Add the Request trigger

On the workflow designer, select **Add a trigger**, if not already selected.
...
![Screenshot shows workflow designer, the selected prompt named Add a trigger, and the gallery for connectors and operation groups with triggers.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/workflow-designer-triggers-gallery.png)

In the **Add a trigger** pane, [follow these general steps to add the **Request** trigger named **When an HTTP request is received**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-trigger).

The following example selects the **Built-in** option so that only the built-in triggers appear:

![Screenshot shows workflow designer, Add a trigger pane, and selected trigger named When an HTTP request is received.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/add-request-trigger.png)
...
![Screenshot shows information pane for the trigger named When an HTTP request is received.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/request-trigger-added-to-designer.png)
...
On the designer, under the **Request** trigger, [follow these general steps to add the **Office 365 Outlook** action named **Send an email (V2)**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action).

If the action doesn't appear in the initial results, next to the connector name, select **See more**, for example:

![Screenshot shows workflow designer and Add an action pane with action search results, and selected See more options.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/add-send-email-action.png)

When the action's authentication pane appears, select **Sign in** to create a connection to your email account.

![Screenshot shows actions named Send an email (V2) with selected sign in button.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/send-email-action-sign-in.png)
...
![Screenshot shows information for the Office 365 Outlook action named Send an email.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/send-email-action-details.png)
...
Some scenarios require a NuGet package-based project for your app to run, for example, when you want to develop and run custom built-in operations. For more information about converting your project to use NuGet, review [Enable built-connector authoring](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-visual-studio-code#enable-built-in-connector-authoring).
...
**.vscode**
Folder
Contains Visual Studio Code-related settings files, such as **extensions.json**, **launch.json**, **settings.json**, and **tasks.json** files.

**Artifacts**
Folder
Contains integration account artifacts that you define and use in workflows that support business-to-business (B2B) scenarios. 

For example, the sample structure includes the following folders: 

- **Maps**: Contains [maps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-maps) to use for XML transformation operations. 

- **Schemas**: Contains [schemas](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-schemas) to use for XML validation operations. 

- **Rules**: Artifacts for [business rules in rules-based engine projects](https://learn.microsoft.com/en-us/azure/logic-apps/rules-engine/rules-engine-overview).
...
- **custom**: Contains .NET Framework custom assemblies. 

For more information about supported assembly types and where to put them in your project, see [Add assemblies to your project](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-visual-studio-code#add-assembly).

**<*WorkflowName*>**
Folder
For each workflow, the <*WorkflowName*> folder includes a **workflow.json** file, which contains that workflow's underlying JSON definition.

**workflow-designtime**
Folder
Contains development environment-related settings files.

**.funcignore**
File
Contains information related to your installed [Azure Functions Core Tools](https://learn.microsoft.com/en-us/azure/azure-functions/functions-run-local).
...
**Important**: The **local.settings.json** file can contain secrets, so make sure that you also exclude this file from your project source control. This file also contains app settings that your logic app needs to work correctly. For reference information, see [Edit app settings and host settings](https://learn.microsoft.com/en-us/azure/logic-apps/edit-app-settings-host-settings#reference-local-settings-json).
...
[Go to the **ngrok** website](https://dashboard.ngrok.com). Sign up for a new account or sign in to your account, if you have one already.
...
From the [**ngrok** download page](https://ngrok.com/download) or [your account dashboard](https://dashboard.ngrok.com/get-started/setup), download the **ngrok** version that you want, and extract the .zip file.
...
Connect the **ngrok** client to your **ngrok** account by running the following command:
...
For more information about these app settings, see [Edit app settings and host settings for Standard logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/edit-app-settings-host-settings#reference-local-settings-json).
...
To debug a stateless workflow more easily, you can enable the run history for that workflow, and then disable the run history when you're done. Follow these steps for Visual Studio Code, or if you're working in the Azure portal, see [Create single-tenant based workflows in the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal#enable-run-history-stateless).
...
Maps
**.xslt**
For more information, see [Add maps for transformations in workflows](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-maps?tabs=standard#visual-studio-code-1).

Schemas
**.xsd**
For more information, see [Add schemas for validation](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-schemas?tabs=standard#visual-studio-code).

Rules
**.xml**
For more information, see [Create an Azure Logic Apps Rules Engine project](https://learn.microsoft.com/en-us/azure/logic-apps/rules-engine/create-rules-engine-project#create-an-azure-logic-apps-rules-engine-project).

Assemblies
- **.dll** (.NET Framework or .NET 8) 

- **.jar** (Java)
A Standard logic app resource can use or reference specific kinds of assemblies, which you can upload to your project in Visual Studio Code. However, you must add them to specific project folders. For more information, see [Add referenced assemblies](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-maps?tabs=standard#visual-studio-code-1). 

**Note**: If you have a NuGet package-based (.NET) logic app project before assemblies support became available, see [Migrate NuGet package-based projects to use assemblies in the **lib** folder](#migrate-nuget-assemblies).
...
![Screenshot shows Explorer pane with project shortcut menu opened from blank area under project.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/convert-logic-app-project.png)
...
![Screenshot shows migrated assemblies and added code in the CSPROJ file.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/migrate-projects-assemblies-visual-studio-code.png)
...
If you haven't already, [convert your extension bundle-based (Node.js) project to a NuGet package-based (.NET) project](#convert-project-nuget).

Review and follow the steps in the article, [Azure Logic Apps Running Anywhere - Built-in connector extensibility](https://techcommunity.microsoft.com/t5/integrations-on-azure/azure-logic-apps-running-anywhere-built-in-connector/ba-p/1921272).
...
If you have a stateless workflow, [enable run history for the workflow](#enable-run-history-stateless) to make debugging easier.

From the **Run** menu, select **Start Debugging** (F5).
...
![Screenshot shows Explorer pane, workflow.json file's shortcut menu with selected option, Overview.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/open-workflow-overview.png)
...
![Screenshot shows workflow overview page with callback URL.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/find-callback-url.png)
...
![Screenshot shows Outlook email as described in the example.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/workflow-app-result-email.png)
...
![Screenshot that shows the workflow's overview page with run status and history.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/post-trigger-call.png)
...
**Running**
The run was triggered and is in progress, but this status can also appear for a run that is throttled due to [action limits](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config?tabs=standard) or the [current pricing plan](https://azure.microsoft.com/pricing/details/logic-apps/). 

**Tip**: If you set up [diagnostics logging](https://learn.microsoft.com/en-us/azure/logic-apps/monitor-workflows-collect-diagnostic-data), you can get information about any throttle events that happen.

**Succeeded**
The run succeeded. If any action failed, a subsequent action in the workflow handled that failure.

**Timed out**
The run timed out because the current duration exceeded the run duration limit, which is controlled by the setting named [**Run history retention in days**](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config?tabs=standard#run-duration-retention-limits). A run's duration is calculated by using the run's start time and run duration limit at that start time. 

**Note**: If the run's duration also exceeds the current *run history retention limit*, which is also controlled by the [**Run history retention in days** setting](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config?tabs=standard#run-duration-retention-limits), the run is cleared from the run history by a daily cleanup job. Whether the run times out or completes, the retention period is always calculated by using the run's start time and *current* retention limit. So, if you reduce the duration limit for an in-flight run, the run times out. However, the run either stays or is cleared from the run history based on whether the run's duration exceeded the retention limit.
...
![Screenshot shows workflow's run history row with selected ellipses button and Show Run.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/show-run-history.png)

Visual Studio Code opens the run details view and shows the status for each step in the workflow run.

![Screenshot shows each step in workflow run and their status.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/run-history-action-status.png)
...
![Screenshot shows status for each step in workflow plus inputs and outputs in expanded action named Send an email.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/run-history-details.png)
...
When you have a workflow that starts with the **When an HTTP request is received** trigger, your workflow can return a response to the caller that sent the initial request by using the [**Request** action named **Response**](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres).

In the workflow designer, under the **Send an email** action, [follow these general steps to add the **Request** action named **Response**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action).

![Screenshot shows workflow designer and Response information pane.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/response-action-details.png)
...
![Screenshot shows open dynamic content list where under Send an email header, the Body output value is selected.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/send-email-body-output.png)
...
![Screenshot shows workflow designer, Response information pane, and Body parameter set to Body output from the action named Send an email.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/response-action-details-complete.png)
...
![Screenshot shows status for each step in updated workflow plus inputs and outputs in expanded Response action.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/run-history-details-rerun.png)
...
If your environment has strict network requirements or firewalls that limit traffic, you have to set up permissions for any connections created by [Azure managed, hosted, and shared connectors](https://learn.microsoft.com/en-us/azure/connectors/managed) and used in your workflow.
...
- [Deploy and configure Azure Firewall using the Azure portal - classic](https://learn.microsoft.com/en-us/azure/firewall/tutorial-firewall-deploy-portal).

## Deploy to Azure

To deploy your Standard logic app from Visual Studio Code, you can directly publish your project to Azure. You can publish your logic app as a new resource, which automatically creates any required resources, such as an [Azure storage account, similar to the function app requirements](https://learn.microsoft.com/en-us/azure/azure-functions/storage-considerations). Or, you can publish your logic app to a previously deployed Standard logic app resource, which overwrites the deployed version.

Deployment for the Standard logic app resource requires a hosting plan and pricing tier, which you select during deployment. For more information, see [Hosting plans and pricing tiers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-pricing#standard-pricing).
...
![Screenshot shows Explorer pane, shortcut menu opened from blank area under project, and selected option for Deploy to logic app.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/deploy-logic-app.png)
...
![Screenshot shows deployment options list and selected option to create a new standard logic app resource with custom steps.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/select-create-logic-app-options.png)
...
![Screenshot shows prompt to enter a unique name for destination logic app.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/deploy-logic-app-name.png)
...
**Note**: Before you select this option, make sure that you have set up the necessary infrastructure. For more information, see [Set up your own infrastructure for Standard logic apps using hybrid deployment](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements).
...
For more information, see [Hosting plans and pricing tiers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-pricing#standard-pricing).
...
If you already set up an SQL database to use for storage by following [Set up SQL database storage for Standard logic app workflows](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-sql-db-storage-single-tenant-standard-workflows), you can select **Yes**.
...
![Screenshot shows confirmation message with Deploy and Cancel options.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/confirm-deploy.png)
...
![Screenshot shows Output window and scope list with Azure Logic Apps Standard selected along with deployment progress and statuses.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/logic-app-deployment-output-window.png)
...
![Screenshot shows a message that logic app creation successfully completed.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/logic-app-creation-completed.png)
...
In the [Azure portal](https://portal.azure.com), go to your Application Insights resource.
...
![Screenshot shows Azure search box with logic app name entered.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/find-deployed-workflow-azure-portal.png)
...
![Screenshot shows logic app deployed from Visual Studio Code and designer with previously created workflow.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/opened-workflow-designer.png)

You can now make changes to this workflow in the Azure portal.

Make sure to [enable the monitoring experience for your deployed logic app](#enable-monitoring) so you can view workflow run history, inputs, outputs, and other related information.
...
In the [Azure portal](https://portal.azure.com), open the deployed Standard logic app resource.
...
![Screenshot shows Azure portal with deployed Standard logic app resource. On the resource menu, CORS is selected with a new entry for Allowed Origins set to the wildcard * character.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/enable-run-history-deployed-logic-app.png)

## Enable or open Application Insights after deployment

During workflow execution, your logic app workflow emits telemetry along with other events. You can use this telemetry to get better visibility into how well your workflow runs and how the Azure Logic Apps runtime works. [Application Insights](https://learn.microsoft.com/en-us/azure/azure-monitor/app/app-insights-overview) provides the capability for you to enable diagnostics logging, tracing, and monitoring for your logic app using near real-time telemetry (live metrics). This capability helps you investigate failures and performance problems more easily when you use the telemetry data to diagnose issues, set up alerts, and build charts.

If you didn't previously set up Application Insights, you can enable this capability in the Azure portal after logic app deployment from Visual Studio Code. You need to have an Application Insights resource in Azure, but you can create this resource [separately in advance](https://learn.microsoft.com/en-us/azure/azure-monitor/app/create-workspace-resource) or when you enable this capability after deployment.
...
In the [Azure portal](https://portal.azure.com), find and open your deployed logic app.
...
Select an existing workspace, if any. Otherwise, a default workspace is created for you. For more information, see [Log Analytics workspace overview](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-analytics-workspace-overview).
...
![Screenshot shows Output window with scope list and Azure Logic Apps (Standard) selected.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/check-output-window-azure-logic-apps.png)
...
See [Supported triggers and actions are missing from the designer in the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal#missing-triggers-actions).
...
To resolve this problem and adjust for the longer URI, edit the **`UrlSegmentMaxCount`** and **`UrlSegmentMaxLength`** registry keys on your computer by following these steps. These key's default values are described in this article, [Http.sys registry settings for Windows](https://learn.microsoft.com/en-us/troubleshoot/iis/httpsys-registry-windows).
...
![Screenshot that shows the registry editor.](https://learn.microsoft.commedia/create-standard-workflows-visual-studio-code/edit-registry-settings-uri-length.png)
...
- [Single-tenant versus multitenant in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/single-tenant-overview-compare#resource-environment-differences)
- [Manage logic apps in Visual Studio Code](https://learn.microsoft.com/en-us/azure/logic-apps/manage-logic-apps-visual-studio-code)
