<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/connect-on-premises-data-sources -->
...
- In Consumption logic app workflows, you can connect to on-premises data sources only by using [connectors that provide access through the on-premises data gateway](https://learn.microsoft.com/en-us/azure/connectors/managed.md#on-premises-connectors).
...
This guide shows how to create the Azure data gateway resource after you [install the on-premises gateway on your local computer](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install).

For more information, see the following documentation:

* [Connectors that can access on-premises data sources](https://learn.microsoft.com/en-us/azure/connectors/managed.md#on-premises-connectors)
* [How the gateway works](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install#gateway-cloud-service)
...
* [Azure Analysis Services on-premises data gateway](https://learn.microsoft.com/en-us/azure/analysis-services/analysis-services-gateway)

<a name="supported-connections"></a>

## Supported data sources

In Azure Logic Apps, the on-premises data gateway supports [on-premises connectors](https://learn.microsoft.com/en-us/azure/connectors/managed.md#on-premises-connectors) for the following data sources:
...
You can also create [custom connectors](https://learn.microsoft.com/en-us/azure/logic-apps/custom-connector-overview) that connect to data sources over HTTP or HTTPS by using REST or SOAP. Although a gateway itself doesn't incur extra costs, the [Azure Logic Apps pricing model](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-pricing) applies to connectors and other Azure Logic Apps operations.
...
* You already [installed an on-premises data gateway on a local computer](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install). This data gateway installation must exist before you can create a data gateway resource that links to this installation. You can install only one data gateway per local computer.

* You have the [same Azure account and subscription](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install#requirements) that you used for your gateway installation. This Azure account must belong only to a single [Microsoft Entra tenant or directory](https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-whatis.md#terminology). You have to use the same Azure account and subscription to create your gateway resource in Azure because only the gateway administrator can create the gateway resource in Azure. Service principals currently aren't supported.
...
1. In the Azure portal search box, enter **on-premises data gateway**, and then select **On-premises data gateways**.
   ...
   ![Screenshot shows Azure portal search box with the words, on-premises data gateway. The results list shows the selected option, On-premises data gateways.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/search-for-on-premises-data-gateway.png)

1. Under **On-premises data gateways**, select **Create**.

   ![Screenshot shows the page for On-premises data gateways with the selected option for Create.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/add-azure-data-gateway-resource.png)
...
1. Under **Create a gateway**, provide the following information for your gateway resource. When you're done, select **Review + create**.
   ...
   | **Resource group** | Select the [Azure resource group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/overview) that you want to use. |
   ...
   | **Region** | Select the same region or location that you selected for the gateway cloud service during [gateway installation](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install). Otherwise, your gateway installation doesn't appear in the **Installation Name** list. Your logic app resource location can differ from your gateway resource location. |
   | **Installation Name** | Select a gateway installation, which appears in the list only when these conditions are met: <p><p>- The gateway installation uses the same region as the gateway resource that you want to create. <br>- The gateway installation isn't linked to another Azure gateway resource. <br>- The gateway installation is linked to the same Azure account that you're using to create the gateway resource. <br>- Your Azure account belongs to a single [Microsoft Entra tenant or directory](https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-whatis.md#terminology) and is the same account that you used for the gateway installation. <p><p>For more information, see [Frequently asked questions](#frequently-asked-questions). |
   ...
   ![Screenshot shows the page for Create a gateway. The Name, Region, and other boxes contain values. The button, Review + create, appears selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/on-premises-data-gateway-create-connection.png)
...
1. Add a trigger or action from a connector that supports on-premises connections through the data gateway.
   ...
   > [!NOTE]
   >
   > In Consumption logic app workflows, if a connector has a [managed version](https://learn.microsoft.com/en-us/azure/connectors/managed.md#on-premises-connectors) 
   > and a [built-in version](https://learn.microsoft.com/en-us/azure/connectors/built-in), use the managed version, which includes the gateway selection capability. 
...
1. For the trigger or action, provide the following information:
   ...
   1. Provide a unique connection name and other required information, which depends on the connection that you want to create.
      ...
      ![Screenshot shows SQL Server managed connector with values in the Connection Name, Authentication Type, and other parameter boxes.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/logic-app-gateway-connection.png)
...
1. To find all the API connections for your Consumption logic app resource, on your logic app menu, under **Development Tools**, select **API connections**.
   ...
   ![Screenshot shows Azure portal, Consumption logic app resource, and resource menu. On resource menu, under Development Tools, API connections is selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/logic-app-api-connections.png)
...
1. Make the changes that you want.
   ...
   > [stopping and restarting the gateway Windows service account](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-install#restart-gateway) 
...
1. On the gateway resource toolbar, select **Delete**.
   ...
   ![Screenshot shows on-premises data gateway resource in the Azure portal. On the toolbar, Delete is selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/connect-on-premises-data-sources/delete-on-premises-data-gateway.png)
...
* Your Azure account doesn't belong to only a single [Microsoft Entra tenant or directory](https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-whatis.md#terminology). Check that you're using the same Microsoft Entra tenant or directory that you used during gateway installation.
...
[Include](https://learn.microsoft.com/en-us/azure/includes/logic-apps-existing-gateway-location-changed.md)

## Next steps

* [Secure your logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app)
* [Common examples and scenarios for logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-examples-and-scenarios)
