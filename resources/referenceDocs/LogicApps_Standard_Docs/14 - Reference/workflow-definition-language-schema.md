<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-schema -->
...
In a workflow definition, the `triggers` and `actions` sections define the calls that happen during your workflow's execution. For syntax and more information about these sections, see [Workflow triggers and actions](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers).
...
Next, create an [Azure Resource Manager template](https://learn.microsoft.com/en-us/azure/azure-resource-manager/templates/overview) for your workflow definition, define template parameters that accept the values you want at deployment, replace hardcoded values with references to template or workflow definition parameters as appropriate, and store the values to use at deployment in a separate [parameter file](https://learn.microsoft.com/en-us/azure/azure-resource-manager/templates/parameter-files). That way, you can change those values more easily through the parameter file without having to update and redeploy your logic app. For information that's sensitive or must be secured, such as usernames, passwords, and secrets, you can store those values in Azure Key Vault and have your parameter file retrieve those values from your key vault. For more information and examples about defining parameters at the template and workflow definition levels, see [Overview: Automate deployment for logic apps with Azure Resource Manager templates](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-azure-resource-manager-templates-overview).

<a name="static-results"></a>

## Static results

In the `staticResults` attribute, define an action's mock `outputs` and `status` that the action returns when the action's static result setting is turned on. In the action's definition, the `runtimeConfiguration.staticResult.name` attribute references the name for the static result definition inside `staticResults`. Learn how you can [test logic app workflows with mock data by setting up static results](https://learn.microsoft.com/en-us/azure/logic-apps/test-logic-apps-mock-data-static-results).
...
| Attribute | Required | Type | Description |
|-----------|----------|------|-------------|
| <*static-result-definition-name*> | Yes | String | The name for a static result definition that an action definition can reference through a `runtimeConfiguration.staticResult` object. For more information, see [Runtime configuration settings](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#runtime-config-options). <br><br>You can use any unique name that you want. By default, this unique name is appended with a number, which is incremented as necessary. |
...
[operators](#operators), [variables](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-create-variables-store-values), explicit values, or constants. In your workflow definition, you can use an expression anywhere in a JSON string value by prefixing the expression with the at-sign (\@). When evaluating an expression that represents a JSON value, the expression body is extracted by removing the \@ character, and always results in another JSON value.

For example, for the previously defined `customerName` property, you can get the property value by using the [**parameters()** function](https://learn.microsoft.com/en-us/azure/logic-apps/expression-functions-reference#parameters) in an expression and assign that value to the `accountName` property:
...
![Screenshot shows workflow designer and expression editor.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-workflow-definition-language/expression-builder.png)
...
> For more information, see [Workflow triggers and actions](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers).
...
To get the output from a workflow run, review your logic app's run history and details in the Azure portal or use the [Workflow REST API](https://learn.microsoft.com/en-us/rest/api/logic/workflows). You can also pass output to external systems, for example, Power BI so that you can create dashboards.
...
Some expressions get their values from runtime actions that might not yet exist when your workflow definition starts to run. To reference or work with these values in expressions, you can use [*functions*](https://learn.microsoft.com/en-us/azure/logic-apps/expression-functions-reference) that the Workflow Definition Language provides.

## Related content

* [Workflow Definition Language actions and triggers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers)
* [Workflow REST API](https://learn.microsoft.com/en-us/rest/api/logic/workflows)
