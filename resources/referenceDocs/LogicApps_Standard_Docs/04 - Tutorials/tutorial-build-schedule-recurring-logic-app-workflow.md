<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/tutorial-build-schedule-recurring-logic-app-workflow -->
...
![Screenshot shows example Consumption workflow that runs with the Recurrence trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/check-travel-time-overview.png)
...
* An email account from an email provider that's supported by Azure Logic Apps, such as Office 365 Outlook or Outlook.com. For other supported email providers, see [Connectors for Azure Logic Apps](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors).
...
1. In the Azure portal search box, enter **logic app**, and select **Logic apps**.
   ...
   ![Screenshot shows Azure portal search box with logic app entered and selected option for Logic apps.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/find-select-logic-apps.png)
...
1. On the **Basics** tab, provide the following information about your logic app resource:
   ...
   ![Screenshot shows Azure portal and creation page for multitenant Consumption logic app and details.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/quickstart-create-example-consumption-workflow/create-logic-app-settings.png)
...
1. Rename the **Recurrence** trigger with the following title: **Check travel time every weekday morning**.
   ...
   ![Screenshot shows workflow designer and information pane for Recurrence trigger with renamed trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/rename-recurrence-schedule-trigger.png)
...
1. In the trigger information box, provide the following information:
   ...
   ![Screenshot shows week-related properties set to values described in the preceding table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/recurrence-trigger-property-values.png)

   This trigger fires every weekday, every 15 minutes, starting at 7:00 AM and ending at 9:45 AM. The **Preview** box shows the recurrence schedule. For more information, see [Schedule tasks and workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence) and [Workflow actions and triggers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#recurrence-trigger).
...
1. If you don't have a Bing Maps connection, you're asked to create a connection. Provide the following connection information, and select **Create**.
   ...
   ![Screenshot shows Bing Maps connection box with the example connection name and Bing Maps API key.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/create-maps-connection.png)
...
1. Now enter the values for the following action's properties:
   ...
   ![Screenshot shows completed action named Get route.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/get-route-action-settings.png)
...
1. Provide the following action information:
   ...
   1. Select inside the **Value** box, which shows the options for the dynamic content list (lightning icon) and expression editor (formula icon), and then select the expression editor.
      ...
      ![Screenshot shows the action named Initialize variable with cursor inside the Value property.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-value-expression-editor.png)
   ...
   1. In the expression editor, enter the following expression: **div(,60)**
      ...
      ![Screenshot shows the expression editor with the expression entered for div(,60).](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-value-expression.png)

   1. Within the expression, put your cursor between the left parenthesis (**(**) and the comma (**,**), and select **Dynamic content**.

      ![Screenshot shows where to put cursor in the div(,60) expression and select Dynamic content.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-dynamic-content.png)
   ...
   1. In the dynamic content list, under select the output value, **Travel Duration Traffic**.
      ...
      ![Screenshot shows the value selected for output named Travel Duration Traffic.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-travel-duration-traffic.png)

   1. After the output value resolves inside the expression, select **Add**.

      ![Screenshot shows selected Add button.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-add.png)

      The following example shows how the **Value** property now appears:

      ![Screenshot shows the Value property with resolved expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/initialize-variable-complete.png)
...
1. Build a condition that checks whether the **travelTime** output value exceeds your specified limit as described and shown here:
   ...
   1. Under **Variables**, select the output named **travelTime**.
      ...
      ![Screenshot shows left box named Choose a value, opened dynamic content list, and travelTime output selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/build-condition-left-side.png)
   ...
   1. On the condition's right side, in the **Choose a value** box, enter the following value: **15**
      ...
      ![Screenshot shows finished condition for comparing the travel time to the specified limit.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/build-condition-check-travel-time.png)
...
1. [Follow these general steps to add an **Office 365 Outlook** action named **Send an email**](create-workflow-with-trigger-or-action.md?tabs=consumption#add-action).
   ...
   * For personal Microsoft accounts, select the **Outlook.com** version.
   ...
   ![Screenshot shows Office 365 Outlook action selected named Send email.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/add-action-send-email.png)
...
1. For the **Body** property, specify the content for the email body by following these steps:
   ...
   1. In the expression editor, enter **sub(,15)** so that you can calculate the number of minutes that exceed your limit:
      ...
      ![Screenshot shows expression editor with the sub(,15) entered.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/send-email-body-expression-editor.png)

   1. Within the expression, put your cursor between the left parenthesis (**(**) and the comma (**,**), and select **Dynamic content**.

      ![Screenshot shows where to put cursor in the sub(,15) expression, and select Dynamic content.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/send-email-body-select-dynamic-content.png)

   1. Under **Variables**, select **travelTime**.

      ![Screenshot shows dynamic content list with travelTime variable selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/send-email-body-select-travel-time.png)

   1. After the variable resolves inside the expression, select **Add**.

      The **Body** property now appears as shown here:

      ![Screenshot shows the resolved expression in the email action's Body property.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/send-email-body-complete.png)

1. Save your workflow. On the designer toolbar, select **Save**.

Next, test and run your workflow, which now looks similar to the following example:

![Screenshot shows complete example logic app workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/check-travel-time-finished.png)
...
* If the current travel time exceeds your limit, you get an email with the current travel time and the number of minutes above your limit. The following example shows a sample email that your workflow sends:
  ...
  ![Screenshot shows example email that reports current travel time and extra travel time that exceeds your specified limit.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-build-scheduled-recurring-logic-app-workflow/received-example-email-notification.png)
  ...
  > see [Troubleshoot your workflow](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-diagnosing-failures).
...
> [Manage mailing list requests](https://learn.microsoft.com/en-us/azure/logic-apps/tutorial-process-mailing-list-subscriptions-workflow)
