<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/tutorial-process-mailing-list-subscriptions-workflow -->
...
![Screenshot shows example Consumption high-level workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/overview.png)
...
* An email account in Office 365 Outlook or Outlook.com, which supports approval workflows. For other email providers, see [Connectors for Azure Logic Apps](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors).
...
* If your logic app workflow needs to communicate through a firewall that limits traffic to specific IP addresses, that firewall needs to allow access for *both* the [inbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#inbound) and [outbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#outbound) IP addresses used by Azure Logic Apps in the Azure region where your logic app resource exists. If your logic app also uses [managed connectors](https://learn.microsoft.com/en-us/azure/connectors/managed), such as the Office 365 Outlook connector or SQL connector, or uses [custom connectors](https://learn.microsoft.com/en-us/connectors/custom-connectors/), the firewall also needs to allow access for *all* the [managed connector outbound IP addresses](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#outbound) in your logic app's Azure region.
...
1. On the **Basics** tab, provide the following information about your logic app resource:
   ...
   ![Screenshot shows Azure portal and creation page for multitenant Consumption logic app and details.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/create-logic-app-settings.png)
...
1. In the trigger information box, from the **Advanced parameters** list, add the following parameters, if they don't appear, and provide the following information:
   ...
   For more information about this trigger's properties, see the [Office 365 Outlook connector reference](https://learn.microsoft.com/en-us/connectors/office365/) or the [Outlook.com connector reference](https://learn.microsoft.com/en-us/connectors/outlook/).

   When you finish, the trigger looks similar to the following example:

   ![Screenshot shows Consumption workflow with trigger named When a new email arrives.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/trigger-information.png)
...
1. For the **Send approval email** action, provide the following information:
   ...
   For more information about these properties, see the [Office 365 Outlook connector reference](https://learn.microsoft.com/en-us/connectors/office365/) or the [Outlook.com connector reference](https://learn.microsoft.com/en-us/connectors/outlook/).

   When you finish, the **Send approval email** action looks like the following example: 

   ![Screenshot shows information for action named Send approval email.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-action-approval-email-settings.png)
...
1. Build a condition that checks whether the approver selected **Approve**.
   ...
   1. On the **Parameters** tab, in the first row under the **AND** list, select inside the left box, and then select the dynamic content list (lightning icon). From this list, in the **Send approval email** section, select the **SelectedOption** output.
      ...
      ![Screenshot shows condition action, second row with cursor in leftmost box, open dynamic content list, and SelectedOption selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/build-condition-selected-option.png)
   ...
   ![Screenshot shows the finished condition for example approval workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/build-condition-done.png)
...
1. In the **Add member to list** action, provide the following information:
   ...
   For more information about the **Add member to list** action properties, see the [MailChimp connector reference](https://learn.microsoft.com/en-us/connectors/mailchimp/).

   When you finish, the **Add member to list** action looks like the following example:

   ![Screenshot shows information for the MailChimp action named Add member to list.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-action-mailchimp-settings.png)
...
1. Build a condition that checks whether the approved member succeeds or fails in joining your mailing list.
   ...
   ![Screenshot shows finished condition to check added member.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/build-condition-check-member-done.png)
...
1. In the **Send email on success** action, provide the following information:
   ...
   ![Screenshot shows information for action named Send email on success.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-action-email-success.png)
...
1. In the **Send email on failure** action, provide the following information:
   ...
   ![Screenshot shows information for action named Send email on failure.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-action-email-failed.png)

1. Save your workflow.

Your finished workflow looks similar to the following example:

![Screenshot shows example finished workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/workflow-done.png)
...
1. If the subscriber's email address doesn't exist on your mailing list, your workflow adds that person's email address and sends you an email like the following example:
   ...
   ![Screenshot shows example email for successful subscription.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-member-success.png)

1. If your workflow can't add the subscriber, you get an email like the following example:

   ![Screenshot shows example email for failed subscription.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-mailing-list-subscriptions-workflow/add-member-failure.png)
  ...
  > [Troubleshoot your logic app](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-diagnosing-failures).
...
> [Process email attachments](https://learn.microsoft.com/en-us/azure/logic-apps/tutorial-process-email-attachments-workflow)
