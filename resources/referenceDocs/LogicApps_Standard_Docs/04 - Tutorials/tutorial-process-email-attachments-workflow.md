<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/tutorial-process-email-attachments-workflow -->
...
![Screenshot shows example Consumption high-level workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/overview.png)
...
* If your workflow needs to communicate through a firewall that limits traffic to specific IP addresses, that firewall needs to allow access for *both* the [inbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#inbound) and [outbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#outbound) IP addresses used by Azure Logic Apps in the Azure region where your logic app resource exists. If your workflow also uses [managed connectors](https://learn.microsoft.com/en-us/azure/connectors/managed), such as the Office 365 Outlook connector or SQL connector, or uses [custom connectors](https://learn.microsoft.com/en-us/connectors/custom-connectors/), the firewall also needs to allow access for *all* the [managed connector outbound IP addresses](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#outbound) in your logic app's Azure region.

## Set up storage to save attachments

The following steps set up [Azure storage](https://learn.microsoft.com/en-us/azure/storage/common/storage-introduction) so that you can store incoming emails and attachments as blobs.

1. In the [Azure portal](https://portal.azure.com), sign in with your Azure account credentials.

1. [Follow these steps to create a storage account](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create.md?tabs=azure-portal) unless you already have one.
   ...
   | **Resource group** | Yes | <*Azure-resource-group*> | The name for the [Azure resource group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/overview) used to organize and manage related resources. <br><br>**Note**: A resource group exists inside a specific region. Although the items in this tutorial might not be available in all regions, try to use the same region when possible. <br><br>This example uses **LA-Tutorial-RG**. |
   ...
   | **Primary service** | No | <*Azure-storage-service*> | The primary storage type to use in your storage account. See [Review options for storing data in Azure](https://learn.microsoft.com/en-us/azure/storage/common/storage-introduction.md#review-options-for-storing-data-in-azure). |
   | **Performance** | Yes | - **Standard** <br>- **Premium** | This setting specifies the data types supported and media for storing data. See [Storage account overview](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-overview). <br><br>This example uses **Standard**. |
   | **Redundancy** | Yes | - **Locally-redundant storage** <br>- **Geo-redundant storage (GRS)** | This setting enables storing multiple copies of your data as protection from planned and unplanned events. For more information, see [Azure Storage redundancy](https://learn.microsoft.com/en-us/azure/storage/common/storage-redundancy). <br><br>This example uses **Geo-redundant storage (GRS)**. |

   To create your storage account, you can also use [Azure PowerShell](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create.md?tabs=powershell) or [Azure CLI](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create.md?tabs=azure-cli).
...
1. Get the storage account's access key by following these steps:
   ...
   [Azure PowerShell](https://learn.microsoft.com/en-us/powershell/module/az.storage/get-azstorageaccountkey)
   or [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/storage/account/keys).
...
To create a storage container, you can also use [Azure PowerShell](https://learn.microsoft.com/en-us/powershell/module/az.storage/new-azstoragecontainer) or [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/storage/container#az-storage-container-create).
...
1. To find your blob storage container, under **Storage Accounts**, expand your storage account, which is **attachmentstorageacct** for this example. Under **Blob Containers** where you find the **attachments** container, for example:
   ...
   ![Screenshot showing Storage Explorer - find storage container.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/storage-explorer-check-contianer.png)
...
1. Before you can create a function, [create a function app by selecting the **Consumption** plan](https://learn.microsoft.com/en-us/azure/azure-functions/functions-create-function-app-portal) and following these steps:
   ...
   1. On the **Basics** tab, provide the following information:
      ...
      | **Runtime stack** | Yes | <*programming-language*> | The runtime for your preferred function programming language. For C# and F# functions, select **.NET**. <br><br>This example uses **.NET**. <br><br>In-portal editing is only available for the following languages: <br><br>- JavaScript <br>- PowerShell <br>- TypeScript <br>- C# script <br><br>You must [locally develop](https://learn.microsoft.com/en-us/azure/azure-functions/functions-develop-local.md#local-development-environments) any C# class library, Java, and Python functions. |
...
1. On the function app menu, select **Overview**. On the **Functions** tab, select **Create**.
   ...
   > [!NOTE]
   >
   > If you don't see the **Create** button, [you must instead create your function locally](https://learn.microsoft.com/en-us/azure/azure-functions/functions-develop-local.md#local-development-environments).

1. [Follow these generic steps to create your function in C# using the **HTTP trigger** template](https://learn.microsoft.com/en-us/azure/azure-functions/functions-create-http-endpoint.md?pivots=programming-language-csharp#create-function), and provide the following information for your function:
...
1. On the **Basics** tab, provide the following information about your logic app resource:
   ...
   > [Azure regions that support availability zones](https://learn.microsoft.com/en-us/azure/reliability/availability-zones-region-support). 
   > For more information, see [Reliability in Azure Functions](https://learn.microsoft.com/en-us/azure/reliability/reliability-functions#availability-zone-support) and 
...
1. In the trigger information box, from the **Advanced parameters** list, add the following parameters, if they don't appear, and provide the following information:
   ...
   ![Screenshot shows Consumption workflow and Office 365 Outlook trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/trigger-information.png)
...
1. Build a condition that checks for emails that have attachments.
   ...
   1. On the **Parameters** tab, in the first row under the **AND** list, select inside the left box, and then select the dynamic content list (lightning icon). From this list, in the trigger section, select the **Has Attachment** output.
      ...
      ![Screenshot shows condition action, second row with cursor in leftmost box, open dynamic content list, and Has Attachment selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/has-attachment.png)
   ...
   1. In the right box, enter **true**, which is the value to compare with the **Has Attachment** output value from the trigger. If both values are equal, the email has at least one attachment, the condition passes, and the workflow continues.
      ...
      ![Screenshot shows complete condition.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/finished-condition.png)
...
1. To check that the trigger fired and the workflow successfully ran, on the logic app menu, select **Overview**.
   ...
   * To view successfully fired triggers, select **Trigger history**.
   ...
   If the trigger didn't fire, or the workflow didn't run despite a successful trigger, see [Troubleshoot your logic app workflow](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-diagnosing-failures).
...
1. Now specify the input for your function to process.
   ...
   1. From the dynamic content list, under **When a new email arrives**, select the **Body** output. After this value resolves in the **Request Body** box, remember to add the closing curly brace (**}**).
      ...
      ![Screenshot shows Azure function information box with dynamic content list and Body selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/add-email-body.png)

   When you finish, the Azure function looks like the following example:

   ![Screenshot shows finished Azure function with request body content to pass to your function.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/add-email-body-done.png)
...
1. Provide the following action information:
   ...
   ![Screenshot shows storage container, sender, and HTML-free email body in Create blob action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/create-blob-email-body.png)

   When you finish, the action looks like the following example:

   ![Screenshot shows example email body information for finished Create blob action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/create-blob-email-body-done.png)
...
1. Send yourself an email that meets the following criteria:
   ...
   * Your email has some test content in the body, for example: **Testing my logic app workflow**
   ...
   If your workflow didn't trigger or run despite a successful trigger, see [Troubleshoot your logic app workflow](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-diagnosing-failures).
...
1. Check that your workflow saved the email to the correct storage container.
   ...
   1. Check the **attachments** container for the email.
      ...
      ![Screenshot shows Storage Explorer with only the saved email.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/storage-explorer-saved-email.png)
...
1. Now select the content for the loop to process.
   ...
   1. From the **When a new email arrives** section, select **Attachments**.
      ...
      ![Screenshot shows dynamic content list with selected output named Attachments.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/select-attachments.png)
...
1. Provide the following action information:
   ...
   ![Screenshot shows storage container and attachment information in Create blob action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/create-blob-per-attachment.png)

   When you finish, the action looks like the following example:

   ![Screenshot shows example attachment information for finished Create blob action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/create-blob-per-attachment-done.png)
...
1. Check that your workflow saved the email and attachments to the correct storage container.
   ...
   1. Check the **attachments** container for both the email and the attachments.
      ...
      ![Screenshot shows Storage Explorer and saved email and attachments.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/storage-explorer-saved-attachments.png)
...
1. Provide the following action information and select the outputs to include in the email:
   ...
   ![Screenshot shows sample email to send.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/send-email-done.png)

1. Save your workflow.

Your finished workflow now looks like the following example:

![Screenshot shows finished workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/complete.png)
...
1. Run your workflow. If successful, your workflow sends you an email that looks like the following example:
   ...
   ![Screenshot shows example email sent by logic app workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/tutorial-process-email-attachments-workflow/email-notification.png)

   If you don't get any emails, check your email's junk folder. Otherwise, if you're unsure that your workflow ran correctly, see [Troubleshoot your logic app workflow](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-diagnosing-failures).
...
> [Learn about connectors in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/introduction)
