...
* [Request](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres)
* [HTTP webhook](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-webhook)
* Managed connector triggers that have the [ApiConnectionWebhook type](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#apiconnectionwebhook-trigger) and can receive inbound HTTPS requests
...
1. Add the **Request** trigger to your workflow by following the [general steps to add a trigger](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow.md?tabs=standard#add-trigger).
...
1. Optionally, in the **Request Body JSON Schema** box, you can enter a JSON schema that describes the payload or data that you expect the trigger to receive.
   ...
   ![Screenshot shows Standard workflow with Request trigger and Request Body JSON Schema parameter with example schema.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-schema-standard.png)
...
1. Save your workflow.
   ...
   ![Screenshot shows Standard workflow, Request trigger, and generated callback URL for endpoint.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/endpoint-url-standard.png)
...
1. Optionally, in the **Request Body JSON Schema** box, you can enter a JSON schema that describes the payload or data that you expect the trigger to receive.
   ...
   ![Screenshot shows Consumption workflow with Request trigger and Request Body JSON Schema parameter with example schema.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-schema-consumption.png)
...
1. Save your workflow.
   ...
   ![Screenshot shows Consumption workflow, Request trigger, and generated callback URL for endpoint.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/endpoint-url-consumption.png)
...
1. To copy the callback URL, you have these options:
   ...
   * Copy the callback URL from your logic app's **Overview** page.
     ...
     1. On the **Overview** page, under **Workflow URL**, move your pointer over the URL, and select the copy icon:
        ...
        ![Screenshot shows Consumption logic app Overview page with workflow URL.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/find-trigger-url-consumption.png)
...
1. To build the `triggerOutputs()` expression that retrieves the parameter value, follow these steps:
   ...
   1. In the expression box, enter the following expression, replacing `parameter-name` with your parameter name, and select **OK**.
      ...
      ![Screenshot shows Standard workflow, Response action, and the triggerOutputs expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-standard.png)

      In the **Body** property, the expression resolves to the `triggerOutputs()` token.

      ![Screenshot shows Standard workflow with Response action's resolved triggerOutputs() expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-token.png)
      ...
      ![Screenshot shows Standard workflow with Response action's resolved expression for parameter name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/resolved-expression-parameter-token.png)
      ...
      ![Screenshot shows Standard workflow with Response action and example triggerOutputs expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-postal-code-standard.png)
...
1. From the **Request** trigger, copy the workflow URL, and paste the URL into another browser window. In the URL, add the parameter name and value to the URL in the following format, and press **Enter**.
   ...
   ![Screenshot shows browser with Standard workflow response from request to callback URL.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/browser-response-callback-url-standard.png)
...
1. To build the `triggerOutputs()` expression that retrieves the parameter value, follow these steps:
   ...
   1. In the expression box, enter the following expression, replacing `parameter-name` with your parameter name, and select **OK**.
      ...
      ![Screenshot shows Consumption workflow, Response action, and the triggerOutputs expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-consumption.png)

      In the **Body** property, the expression resolves to the `triggerOutputs()` token.

      ![Screenshot shows Consumption workflow with Response action's resolved triggerOutputs expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-token.png)

      If you save the workflow, navigate away from the designer, and return to the designer, the token shows the parameter name that you specified, for example:

      ![Screenshot shows Consumption workflow with Response action's resolved expression for parameter name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/resolved-expression-parameter-token.png)
      ...
      ![Screenshot shows Consumption workflow with Response action and example triggerOutputs expression.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/trigger-outputs-expression-postal-code-consumption.png)
...
1. From the **Request** trigger, copy the workflow URL, and paste the URL into another browser window. In the URL, add the parameter name and value following the question mark (`?`) to the URL in the following format, and press **Enter**.
   ...
   ![Screenshot shows browser with Consumption workflow response from request to callback URL.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/browser-response-callback-url-consumption.png)
...
1. In the **Request** trigger, open the **Advanced parameters** list, and select **Relative path**, which adds this property to the trigger.
   ...
   ![Screenshot shows Standard workflow, Request trigger, and added property named Relative path.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/add-relative-path-standard.png)
...
1. In the **Relative path** property, specify the relative path for the parameter in your JSON schema that you want your URL to accept, for example, `/address/{postalCode}`.
   ...
   ![Screenshot shows Standard workflow, Request trigger, and Relative path parameter value.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/relative-path-url-standard.png)
...
1. In the **Response** action's **Body** property, include the token that represents the parameter that you specified in your trigger's relative path.
   ...
   1. Select the lightning icon to open the dynamic content list. From the **When a HTTP request is received** section, select the **postalCode** trigger output.
      ...
      ![Screenshot shows Standard workflow, Response action, and specified trigger output to include in response body.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/response-trigger-output-standard.png)

      The **Body** property now includes the selected parameter:

      ![Screenshot shows Standard workflow and example response body with parameter.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/response-parameter-standard.png)
...
1. In the **Request** trigger, open the **Advanced parameters** list, and select **Relative path**, which adds this property to the trigger.
   ...
   ![Screenshot shows Consumption workflow, Request trigger, and added property named Relative path.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/add-relative-path-consumption.png)

1. In the **Relative path** property, specify the relative path for the parameter in your JSON schema that you want your URL to accept, for example, `/address/{postalCode}`.

   ![Screenshot shows Consumption workflow, Request trigger, and Relative path parameter value.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/relative-path-url-consumption.png)
...
1. In the **Response** action's **Body** property, include the token that represents the parameter that you specified in your trigger's relative path.
   ...
   1. Select the lightning symbol to open the dynamic content list. From the **When a HTTP request is received** section, select the **postalCode** trigger output.
      ...
      ![Screenshot shows Consumption workflow, Response action, and specified trigger output to include in response body.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/response-trigger-output-consumption.png)

      The **Body** property now includes the selected parameter:

      ![Screenshot shows Consumption workflow and example response body with parameter.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/response-parameter-consumption.png)
...
1. From the **Workflow Name** list, select the workflow that you want to call, for example:
   ...
   ![Screenshot shows Standard workflow, action named Invoke a workflow in this workflow app, opened Workflow Name list, and available workflows to call.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/select-workflow-standard.png)
...
1. From the **Choose an operation** box, select an available workflow that you want to call, for example:
   ...
   ![Screenshot shows Consumption workflow with Choose an operation box and available workflows to call.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/select-workflow-consumption.png)
...
For example, if you're passing content that has `application/xml` type, you can use the [`xpath()` expression](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#xpath) to perform an XPath extraction, or use the [`json()` expression](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#json) for converting XML to JSON. Learn more about working with supported [content types](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-content-type).

To get the output from an incoming request, you can use the [`triggerOutputs` expression](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#triggerOutputs). For example, suppose you have output that looks like this example:
...
To specifically access the `body` property, you can use the [`triggerBody()` expression](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#triggerBody) as a shortcut.
...
![Screenshot shows Response action and response content type.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-http-endpoint/content-for-response-action.png)
...
For more information about security, authorization, and encryption for inbound calls to your workflow, such as [Transport Layer Security (TLS)](https://en.wikipedia.org/wiki/Transport_Layer_Security), [Microsoft Entra ID Open Authentication (Microsoft Entra ID OAuth)](https://learn.microsoft.com/en-us/azure/active-directory/develop/index.yml), exposing your logic app workflow with Azure API Management, or restricting the IP addresses that originate inbound calls, see [Secure access and data - Access for inbound calls to request-based triggers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app#secure-inbound-requests).

#### Can I configure callable endpoints further?

Yes, HTTPS endpoints support more advanced configuration through [Azure API Management](https://learn.microsoft.com/en-us/azure/api-management/api-management-key-concepts). This service also offers the capability for you to consistently manage all your APIs, including logic apps, set up custom domain names, use more authentication methods, and more, for example:

* [Set request method](https://learn.microsoft.com/en-us/azure/api-management/set-method-policy)
* [Rewrite URL](https://learn.microsoft.com/en-us/azure/api-management/rewrite-uri-policy)
...
* [Receive and respond to inbound HTTPS calls to workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres)
* [Secure access and data in Azure Logic Apps - Access for inbound calls to request-based triggers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app#secure-inbound-requests)
