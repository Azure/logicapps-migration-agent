<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-create-variables-store-values -->
...
> run the loop [sequentially](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-control-flow-loops.md#sequential-foreach-loop) 
...
- A logic app workflow where you want to create the variable and a trigger that starts the workflow.
  ...
  Before you can add actions for creating and working with variables, your workflow must start with a trigger as the first step in your workflow. For more information, see [Add a trigger or action to build a workflow](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action).
...
1. Provide the following information about your variable:
   ...
   ![Screenshot shows Azure portal and a logic app workflow with an action named Initialize variables.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/initialize-variable.png)
...
To retrieve or reference a variable's contents, you can use the [`variables()` function](https://learn.microsoft.com/en-us/azure/logic-apps/expression-functions-reference#variables) in the workflow designer and the code view editor. When you reference a variable, use the variable's name as the token, not the action's name, which is the usual way to reference an action's outputs.
...
1. Provide the following information for this action:
   ...
   ![Screenshot shows Azure portal and a logic app workflow with an action named Increment variable.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/increment-variable-action.png)
...
1. In the trigger, to check for attachments and pass those attachments into your workflow, select **Yes** for the following properties:
   ...
   ![Screenshot shows  Azure portal and logic app workflow with Office 365 Outlook trigger properties selected to check for and include attachments.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/check-include-attachments.png)
...
1. In the loop, select inside the box named **Select an Output from Previous Steps** to view the input options. Select the lightning icon to view the dynamic content list. From the list, under **When a new email arrives**, select **Attachments**.
   ...
   ![Screenshot shows Azure portal and logic app workflow with a For each loop, opened dynamic content list, and selected Attachments output.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/select-attachments.png)
...
1. In the **Increment variable** action, from the **Name** list, select the **Count** variable. Set the **Value** property to **1**.
   ...
   ![Screenshot Azure portal and logic app workflow with a selected variable named Count.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/add-increment-variable-loop.png)
...
1. Under the loop, add any action that sends you the number of attachments. In your action, include the value from the **Count** variable, for example:
   ...
   ![Screenshot shows Azure portal and logic app workflow with an action that sends results.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-create-variables-store-values/send-email-results.png)
