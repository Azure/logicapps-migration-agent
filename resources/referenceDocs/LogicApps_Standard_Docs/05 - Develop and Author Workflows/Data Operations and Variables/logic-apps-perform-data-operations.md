<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-perform-data-operations -->
...
* The logic app workflow where you want to perform the data operation. Both Consumption and Standard logic app workflows support the data operations described in this guide.
  ...
  All data operations are available only as actions. So, before you can use these actions, your workflow must already start with a [trigger](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview#logic-app-concepts) as the first step and include any other actions required to create the outputs that you want to use in the data operation.
...
To create more complex JSON transformations, see [Perform advanced JSON transformations with Liquid templates](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-enterprise-integration-liquid-transform).
...
To try the **Compose** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Compose** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Compose](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#compose-action-example). For more information about the **Compose** action in the underlying JSON workflow definition, see the [Compose action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#compose-action).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and example workflow for Compose action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-compose-action-consumption.png)
...
1. On the designer, select the **Compose** action, if not already selected. In the **Inputs** box, enter the inputs to use for creating the output.
   ...
   1. In the JSON object, put your cursor in the corresponding locations, select the dynamic content list (lightning icon), and then select the corresponding variable from the list:
      ...
      ![Screenshot shows Consumption workflow, Compose action, dynamic content list, and selected inputs to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-compose-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for Compose action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-compose-action-standard.png)
...
1. On the designer, select the **Compose** action, if not already selected. In the **Inputs** box, enter the inputs to use for creating the output.
   ...
   1. For each variable declaration in the object syntax, place the insertion cursor as described in the following table. Select the dynamic content list (lightning icon), and then select the corresponding definition from the **Variables** list:
      ...
      ![Screenshot shows Standard workflow, Compose action, insert cursor location, and lightning icon.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/compose-lightning-insert-cursor.png)

      The following example shows definitions added for two variables, and the process to add the third variable: 

      ![Screenshot shows Standard workflow, Compose action, dynamic content list, and selected inputs to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-compose-action.png)
...
1. In your workflow, add an action that can send you the results from the **Compose** action.
   ...
   ![Screenshot shows Standard workflow, add action, Office 365 Outlook, Send an email (V2).](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/add-outlook-action-send-email.png)
...
1. In this action, for each box where you want the results to appear, select inside each box, and then select the dynamic content list (lightning icon). In the list, under the **Compose** action, select **Outputs**.
   ...
   ![Screenshot shows workflow designer, the action named Send an email, and output from the preceding Compose action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-compose-action.png)
...
![Screenshot shows email with results from the Compose action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/compose-email-results.png)
...
To try the  **Create CSV table** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Create CSV table** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Create CSV table](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#create-csv-table-action-example). For more information about the **Create CSV table** action in the underlying JSON workflow definition, see the [Table action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#table-action).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Consumption workflow designer, and example workflow for action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-create-table-action-consumption.png)
...
1. On the designer, select the **Create CSV table** action, if not already selected. In the **From** box, enter the array or expression to use for creating the table.
   ...
   ![Screenshot shows Consumption workflow, action named Create CSV table, and the selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-create-csv-table-action.png)
   ...
   ![Screenshot shows Consumption workflow and finished example action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-create-csv-table-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-create-table-action-standard.png)
...
1. On the designer, select the **Create CSV table** action, if not already selected. In the **From** box, enter the array or expression to use for creating the table.
   ...
   ![Screenshot shows Standard workflow, action named Create CSV table, and the selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-create-csv-table-action.png)
   ...
   ![Screenshot shows Standard workflow and finished example action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-create-csv-table-action.png)
...
1. If the **Columns** property doesn't appear in the action information box, from the **Advanced parameters** list, select **Columns**.
   ...
   ![Screenshot shows Standard workflow and Columns parameter selected in the Advanced parameters list.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/advanced-parameters-show-columns.png)
...
To return values from the array, you can use the [**`item()`** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item) with the **Create CSV table** action. In a **`For_each`** loop, you can use the [**`items()`** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#items).
...
1. For each array property that you want, in the **Value** column, select inside the edit box, and then select the function icon, which opens the expression editor. Make sure that the **Function** list appears selected.
   ...
   ![Screenshot shows Standard workflow, Columns parameter, Function expression selected for the Column Value field.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/select-function-expression-value-field.png)
...
1. In the expression editor, enter the following expression, but replace `<array-property-name>` with the array property name for the value that you want. When you're done with each expression, select **Add**.
   ...
   * `item()?['Product_ID']`
   ...
   ![Screenshot shows workflow designer, action named Create CSV table, and how to dereference array property named Description.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/csv-table-expression.png)

   For more information, see [**item()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item).

1. Repeat the preceding steps for each array property. When you're done, your action looks similar to the following example:

   ![Screenshot shows action named Create CSV table and function named item().](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-csv-expression.png)
...
1. In this action, for each box where you want the results to appear, select inside each box, which opens the dynamic content list. From that list, under the **Create CSV table** action, select **Output**.
   ...
   ![Screenshot shows workflow with action named Send an email. The Body property contains the field named Output from preceding action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-create-csv-table-action.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**.

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from action named Create CSV table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/create-csv-table-email-results.png)
...
To try the **Create HTML table** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Create HTML table** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Create HTML table](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#create-html-table-action-example). For more information about the **Create HTML table** action in the underlying JSON workflow definition, see the [Table action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#table-action).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and sample workflow for action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-create-table-action-consumption.png)
...
1. On the designer, select the **Create HTML table** action, if not already selected. In the **From** box, enter the array or expression to use for creating the table.
   ...
   ![Screenshot shows Consumption workflow, action named Create HTML table, and the selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-create-html-table-action.png)
   ...
   ![Screenshot shows Consumption workflow and finished example action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-create-html-table-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and sample workflow for action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-create-table-action-standard.png)
...
1. On the designer, select the **Create HTML table** action, if not already selected. In the **From** box, enter the array or expression to use for creating the table.
   ...
   ![Screenshot shows Standard workflow, action named Create HTML table, and the selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-create-html-table-action.png)
   ...
   ![Screenshot shows Standard workflow and finished example action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-create-html-table-action.png)
...
To return values from the array, you can use the [`item()` function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item) with the **Create HTML table** action. In a `For_each` loop, you can use the [`items()` function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#items).
...
1. In the expression editor, enter the following expression, but replace `<array-property-name>` with the array property name for the value that you want. When you're done with each expression, select **Add**.
   ...
   * `item()?['Product_ID']`
   ...
   ![Screenshot shows workflow designer, action named Create HTML table, and how to dereference array property named Description.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/html-table-expression.png)

   For more information, see [**item()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item).

1. Repeat the preceding steps for each array property. When you're done, your action looks similar to the following example:

   ![Screenshot shows action named Create HTML table and function named item().](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-html-expression.png)
...
1. In this action, for each box where you want the results to appear, select inside each box, and then select the lightning icon, which opens the dynamic content list. From that list, under the **Create HTML table** action, select **Output**.
   ...
   ![Screenshot shows workflow with action named Send an email. The Body property contains the Output field from preceding action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-create-html-table-action.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**.

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from action named Create HTML table.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/create-html-table-email-results.png)
...
To try the **Filter array** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Filter array** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Filter array](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#filter-array-action-example). For more information about the **Filter array** action in the underlying JSON workflow definition, see the [Query action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#query-action).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and example workflow for action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-filter-array-action-consumption.png)
...
1. On the designer, select the **Filter array** action, if not already selected. In the **From** box, enter the array or expression to use as the filter.
   ...
   ![Screenshot shows Consumption workflow, action named Filter array, and selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-filter-array-action.png)

1. For the condition, specify the array items to compare, select the comparison operator, and specify the comparison value.

   This example uses the [**item()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item) to access each item in the array, while the **Filter array** action searches for array items where the value is greater than one. The following screenshot shows the finished example **Filter array** action:

   ![Screenshot shows Consumption workflow and finished example action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-filter-array-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-filter-array-action-standard.png)
...
1. On the designer, select the **Filter array** action, if not already selected. In the **From** box, enter the array or expression to use as the filter.
   ...
   ![Screenshot shows Standard workflow, action named Filter array, and selected input to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-filter-array-action.png)

1. For the condition, specify the array items to compare, select the comparison operator, and specify the comparison value.

   This example uses the [**item()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item) to access each item in the array, while the **Filter array** action searches for array items where the value is greater than one. The following screenshot shows the finished example **Filter array** action:

   ![Screenshot shows Standard workflow and finished example action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-filter-array-action.png)
...
1. In this action, complete the following steps:
   ...
   1. To get the array output from the **Filter array** action, enter the following expression, which uses the [**body()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#body) with the **Filter array** action name, and then select **Add**.

      `body('Filter_array')`

      ![Screenshot shows workflow with action named Send an email. The Body property contains the body() function, which gets the body content from the preceding action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-filter-array-action.png)

      The resolved expression specifies to show the outputs from the **Filter_array** action in the email body when sent:

      ![Screenshot shows Standard workflow and finished example action for Send an email.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-filter-array-action-complete.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**.

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from action named Filter array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/filter-array-email-results.png)
...
To try the **Join** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Join** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Join](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#join-action-example). For more information about the **Join** action in your underlying workflow definition, see the [Join action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#join-action).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and example workflow for the action named Join.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-join-action-consumption.png)
...
1. In the **From** box, enter the array that has the items you want to join as a string.
   ...
   ![Screenshot shows Consumption workflow, action named Join, and selected array output to join as a string.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-join-action.png)
...
1. In the **Join With** box, enter the character to use for separating each array item.
   ...
   ![Screenshot shows Consumption workflow and the finished example for the action named Join.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-join-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for the action named Join.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-join-action-standard.png)
...
1. On the designer, select the **Join** action, if not already selected. In the **From** box, enter the array that has the items that you want to join as a string.
   ...
   ![Screenshot shows Standard workflow, action named Join, and selected array output to join as a string.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-join-action.png)

1. In the **Join With** box, enter the character to use for separating each array item.

   This example uses a colon (**:**) as the separator for the **Join With** property.

   ![Screenshot shows Standard workflow and the finished example for the action named Join.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-join-action.png)
...
1. In this action, for each box where you want the results to appear, select inside each box, and then select the lightning icon, which opens the dynamic content list. From that list, under the **Join** action, select **Output**.
   ...
   ![Screenshot shows a workflow with the finished action named Send an email for the Join action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-join-action-complete.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**. 

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from the Join action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/join-email-results.png)
...
To try the **Parse JSON** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Parse JSON** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Parse JSON](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#parse-json-action-example). 
For more information about this action in your underlying workflow definition, see [Parse JSON action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and example workflow for action named Parse JSON.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-parse-json-action-consumption.png)
...
1. On the designer, select the **Parse JSON** action, if not already selected. In the **Content** box, enter the JSON object that you want to parse.
   ...
   ![Screenshot shows Consumption workflow, action named Parse JSON, and the selected JSON object variable to parse.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-parse-json-action.png)
...
1. In the **Schema** box, enter the JSON schema that describes the JSON object, or *payload*, that you want to parse.
   ...
   ![Screenshot shows Consumption workflow, action named Parse JSON, and JSON schema for the JSON object that you want to parse.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/provide-schema-parse-json-action.png)
   ...
   1. In the **Enter or paste a sample JSON payload** box, enter the JSON content or object, and then select **Done**, for example:
      ...
      ![Screenshot shows Consumption workflow, action named Parse JSON, and box named Enter or paste a sample JSON payload, which contains JSON sample to generate the schema.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/generate-schema-parse-json-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for action named Parse JSON.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-parse-json-action-standard.png)
...
1. On the designer, select the **Parse JSON** action, if not already selected. In the **Content** box, enter the JSON object that you want to parse.
   ...
   ![Screenshot shows Standard workflow, action named Parse JSON, and the selected JSON object variable to parse.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-parse-json-action.png)
...
1. In the **Schema** box, enter the JSON schema that describes the JSON object, or *payload*. that you want to parse.
   ...
   ![Screenshot shows Standard workflow, action named Parse JSON, and JSON schema for the JSON object that you want to parse.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/provide-schema-parse-json-action.png)
   ...
   1. In the **Enter or paste a sample JSON payload** box, enter the JSON content or object, and then select **Done**, for example:
      ...
      ![Screenshot shows Standard workflow, action named Parse JSON, and box named Enter or paste a sample JSON payload, which contains JSON sample to generate the schema.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/generate-schema-parse-json-action.png)
...
1. In this action, for each box where you want the results to appear, select inside each edit box, and then select the lightning icon, which opens the dynamic content list. From that list, under the **Parse JSON** action, select the properties from the parsed JSON object.
   ...
   ![Screenshot shows Standard workflow with JSON properties in the action named Send an email.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-parse-json-action.png)

   When you're done, the **Send an email** action looks similar to the following example:

   ![Screenshot shows workflow with finished action named Send an email for action named Parse JSON.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-parse-json-action-complete.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**. 

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from action named Parse JSON.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/parse-json-email-results.png)
...
To try the **Select** action, follow these steps by using the workflow designer. Or, if you prefer working in the code view editor, you can copy the example **Select** and **Initialize variable** action definitions from this guide into your own logic app's underlying workflow definition: [Data operation code examples - Select](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#select-action-example). For more information about this action in your underlying workflow definition, see [Select action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-workflow-actions-triggers#select-action).
...
> [Data operation code examples - Select](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-data-operations-code-samples#select-action-example).
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Consumption workflow designer, and example workflow for the action named Select.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-select-action-consumption.png)
...
1. On the designer, select the **Select** action, if not already selected. In the **From** box, enter the source array that you want to use.
   ...
   ![Screenshot shows Consumption workflow, action named Select, and the selected source array variable to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-select-action.png)
...
1. In the right column, enter an expression that specifies the value in the source array for the property name in the left column.
   ...
   This example uses the [**item()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#item) to iterate through and access each item in the array.
   ...
   1. In the expression editor, enter the function named **item()**, and then select **Add**.
      ...
      ![Screenshot shows Consumption workflow, the action named Select, and the JSON object property and values to create the JSON object array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-select-action-expression.png)

   The **Select** action now appears similar to the following example:

   ![Screenshot shows Consumption workflow and the finished example action named Select.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-select-action.png)
...
1. In the [Azure portal](https://portal.azure.com) or Visual Studio Code, open your logic app workflow in the designer.
   ...
   ![Screenshot shows Azure portal, Standard workflow designer, and example workflow for the action named Select.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/sample-start-select-action-standard.png)
...
1. On the designer, select the **Select** action, if not already selected. In the **From** box, enter the source array that you want to use.
   ...
   ![Screenshot shows Standard workflow, action named Select, and the selected source array variable to use.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-select-action.png)
...
1. In the right column, enter an expression that specifies the value in the source array for the property name in the left column.
   ...
   1. In the expression editor, enter the function named **item()**, and then select **Add**.
      ...
      ![Screenshot shows Standard workflow, the action named Select, and the JSON object property and values to create the JSON object array.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/configure-select-action-expression.png)

   The **Select** action now appears similar to the following example:

   ![Screenshot shows Standard workflow and the finished example action named Select.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/finished-select-action.png)
...
1. In this action, complete the following steps:
   ...
   1. To get the array output from the **Select** action, enter the following expression, which uses the [**body()** function](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference#body) with the **Select** action name, and select **Add**:

      `body('Select')`

      ![Screenshot shows workflow with action named Send an email, and action outputs from the Select action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-select-action.png)

      The resolved expression specifies to show the outputs from the **Select** action in the email body when sent:

      ![Screenshot shows workflow and finished action named Send an email for the Select action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/send-email-select-action-complete.png)

1. Save your workflow, and then manually run your workflow. On the designer toolbar, select **Run** > **Run**. 

If you use the Office 365 Outlook - Send an email action, the following example shows the result:

![Screenshot shows email with results from the Select action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-perform-data-operations/select-email-results.png)
