<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/send-related-messages-sequential-convoy -->
...
Some scenarios require your workflow to process session-related messages in a specific order from an Azure Service Bus [queue](https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-queues-topics-subscriptions). These messages have a property that defines the relationship with each other, such as a [*session* ID](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-sessions). To process these messages by session, set up a [*sequential convoy* pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/sequential-convoy) in your workflow.
...
![Diagram shows the general sequential convoy pattern.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/sequential-convoy-pattern-general.png)
...
1. When the Service Bus trigger named **When a message is received in a queue (peek-lock)** fires, run the workflow.
   ...
   For more information about *peek-lock* mode, see [Receive modes](https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-queues-topics-subscriptions.md#receive-modes) and [Peek lock](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-transfers-locks-settlement.md#peeklock).
...
- A Service Bus namespace (Standard or Premium pricing tier) and a Service Bus *session-enabled* queue.
  ...
  For more information, see [Create a Service Bus namespace and queue](https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-create-namespace-portal).
...
1. On the sidebar menu, under **Settings**, select **Shared access policies**. Check that you have **Manage** permissions for that namespace.
   ...
   ![Screenshot shows the Shared access policies page for a Service Bus namespace.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/check-service-bus-permissions.png)
...
1. Get the connection string for your Service Bus namespace by following these steps:
   ...
   1. On the **SAS Policy: RootManageSharedAccessKey** pane, next to the primary connection string, select the copy button.
      ...
      ![Screenshot shows the Shared access policies where you can copy a Service Bus namespace connection string.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/copy-service-bus-connection-string.png)
...
1. Under **Templates**, select the template named **Azure Service Bus: Process related messages from a session-enabled queue for the same workflow instance - Sequential convoy pattern**, and then select **Use this template**.
   ...
   ![Screenshot shows a page where you can select a service bus sessions template.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/select-workflow-template.png)
...
1. On the **Create a new workflow from template** pane, follow these steps:
   ...
   1. On the **Connections** tab, follow these steps:
      ...
      1. For Service Bus, in the **Connection** column, select **Connect**.
         ...
         ![Screenshot shows pane to start connecting to Azure Service Bus namespace.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/connect-service-bus.png)
      ...
      1. Provide the following information:
         ...
         | **Connection String** | <*namespace-connection-string*> | The connection string that you saved earlier from your Service Bus namespace. |
          ...
          ![Screenshot shows Service Bus connection pane in Standard workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/service-bus-connection-standard.png)
   ...
   ![Screenshot shows workflow with all operations.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/workflow-from-template.png)
   ...
   For more information, see [Create cross-environment parameters for workflow inputs](https://learn.microsoft.com/en-us/azure/logic-apps/create-parameters-workflows).
...
This Azure Service Bus built-in trigger waits for one or more messages to arrive in a specified Service Bus queue. If a message exists in the queue, the trigger fires and runs a workflow instance. The term *peek-lock* means that the trigger sends a request to retrieve a message from the queue. If a message exists, the trigger retrieves and locks the message so that no other processing happens on that message until the lock period expires. The trigger also outputs a [ServiceBusMessage object](https://learn.microsoft.com/en-us/connectors/servicebus/#servicebusmessage).
...
This [**Initialize variable** action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-create-variables-store-values#initialize-variable) named **Initialize Process Complete Flag** creates a variable with the following initial values:
...
The outer parent [**Scope** action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-control-flow-run-steps-group-scopes) contains an inner nested **Scope** action named **Business Logic Scope**. This nested scope contains a placeholder action named **Business Logic**. Here, you can add the actions for your scenario's business logic to process the current message.

1. For clarity, rename the outer parent [**Scope** action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-control-flow-run-steps-group-scopes) to `Process message`.

   ![Screenshot shows the parent scope renamed as Process message.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/rename-parent-scope.png)
...
![Screenshot shows the Run after settings for the Process Finished action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/run-after-settings-template.png)
...
![Screenshot shows the updated values in the Process Finished action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/process-finished-updated.png)
...
![Screenshot shows the parallel branch with the Until loop.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/parallel-branch-until-loop-standard.png)

##### Renew lock on a message in a queue

In the **Until** loop, the Service Bus action named **Renew lock on a message in a queue** controls the time that the workflow can process a message before the message becomes available again for processing. This behavior prevents premature release due to long-running tasks and makes sure that only one client handles the message at a time before setting the lock duration to the initial value. For more information, see [Renew locks](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-transfers-locks-settlement.md#renew-locks).
...
- Branch 1
  ...
  1. Make sure that the error-handling action directly under the `Process message` scope runs only after the `Process message` scope completes with one of the following statuses:
     ...
     ![Screenshot shows error handling scope and run after settings.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/error-handling-consumption.png)
  ...
  ![Screenshot shows Abandon message action for Standard workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/abandon-message-standard.png)
...
- Branch 2
  ...
  ![Screenshot shows Complete message action for Standard workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/complete-message-standard.png)
...
Follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/create-parameters-workflows) to create the cross-environment parameters in this table:
...
![Screenshot shows cross-environment parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/workflow-parameters.png)
...
1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-trigger) to add a Service Bus trigger named **When one or more messages arrive in a queue (peek-lock)**.
...
1. On the connection pane, provide the following information:
   ...
   ![Screenshot shows Service Bus connection pane in Consumption workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/service-bus-connection-consumption.png)
...
1. After the trigger information pane opens, on the **Parameters** tab, provide the following values:
   ...
   ![Screenshot shows the completed Service Bus trigger.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/when-messages-arrive-trigger.png)

   The trigger outputs a [ServiceBusMessage object](https://learn.microsoft.com/en-us/connectors/servicebus/#servicebusmessage).
   ...
   For more information, see [Service Bus - When one or more messages arrive in a queue (peek-lock)](https://learn.microsoft.com/en-us/connectors/servicebus/#when-one-or-more-messages-arrive-in-a-queue-(peek-lock)).
...
1. In the designer, under the trigger, follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add a **Variables** action named **Initialize variables**.
...
1. In the action information pane, provide the following values:
   ...
   ![Screenshot shows the information for Vitalize Process Complete Flag.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/initialize-process-complete.png)
...
These steps add a [**Scope** action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-control-flow-run-steps-group-scopes) to the workflow. Inside this **Scope** action, another scope contains the actions for message processing, based on your scenario's business logic. If a problem happens in these scopes, the actions outside the outermost **Scope** action handle that problem.

1. In the designer, under the `Initialize Process Complete Flag` action, follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add a **Scope** action.

1. Rename the scope action to `Process message`, for example:

   ![Screenshot shows the workflow with the Process Message scope.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/process-message-scope.png)
...
1. In the **Process message** scope, add another **Scope** action.
   ...
   1. Change the second scope name to `Business Logic Scope`, for example:
      ...
      ![Screenshot shows the workflow with Business Logic Scope.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/business-logic-scope.png)
...
1. Outside `Business Logic Scope`, add a **Variables** action named **Set variable**.
   ...
   1. Select all the unselected statuses: **Has timed out**, **Is skipped**, and **Has failed**, for example:
      ...
      ![Screenshot shows the run after settings for Process Finished.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/run-after-settings.png)
...
1. At the same level as the **Business Logic Scope**, add a parallel branch to determine whether any other messages exist in the same session.
   ...
   1. In the **Add an action** pane, add a **Control** action named **Until**, which is a type of loop, for example:
      ...
      ![Screenshot shows the parallel branch with the Until loop.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/parallel-branch-until-loop.png)
   ...
   1. On the **Until** action information pane, provide the condition that stops running the loop.
      ...
      1. When you're done, select **Add**.
         ...
         ![Screenshot shows the resolved expression for processCompleted variable.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/process-completed-until-loop.png)
      ...
      1. In the rightmost **Chose a value** box, enter the value `true`.
         ...
         ![Screenshot shows the completed Until loop.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/until-loop-completed.png)

   1. Add actions for message detection to the **Until** loop by following these steps:

      1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow#add-action) to add a Service Bus action named **Renew lock on the message in a queue**.

         This action extends the time that the workflow can process a message before the message becomes available again for processing. This behavior prevents premature release due to long-running tasks and makes sure that only one client handles the messages at a time before letting the lock duration to the initial value. For more information, see [Renew locks](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-transfers-locks-settlement.md#renew-locks).
      ...
      1. After the Service Bus action information pane opens, provide the following values:
         ...
         ![Screenshot shows the completed renew lock action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/renew-lock-on-message-complete.png)
      ...
      1. Under the Service Bus action, add a **Schedule** action named **Delay**.
         ...
         ![Screenshot shows the completed delay duration.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/wait-process-complete.png)

      When you're done, the **Until** loop looks like the following example:

      ![Screenshot shows the actions in the Until loop.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/until-loop-actions.png)
...
- Branch 1
  ...
  1. Under the error-handling actions, add the Service Bus action named **Abandon the message in a queue**, and then set up the action as follows:
     ...
     ![Screenshot shows Abandon message in queue for Consumption workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/abandon-message-consumption.png)
...
- Branch 2: Add the Service Bus action named **Complete the message in a queue**. This action runs only if the `Process message` scope successfully completes. Set up the action as follows:
  ...
  ![Screenshot shows Complete message in queue for Consumption workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/send-related-messages-sequential-convoy/complete-message-consumption.png)
...
- [Service Bus connector operations](https://learn.microsoft.com/en-us/connectors/servicebus/)
