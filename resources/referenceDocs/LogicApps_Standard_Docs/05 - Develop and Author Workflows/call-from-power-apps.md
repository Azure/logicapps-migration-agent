<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/call-from-power-apps -->
...
1. On your logic app menu, select **Overview**. On the **Overview** page toolbar, select **Export** > **Export to Power Apps**.
   ...
   ![Screenshot shows Azure portal and Overview toolbar with Export button selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-from-power-apps/export-logic-app.png)
...
* **The current Logic App cannot be exported. To export, select a Logic App that has a request trigger.**: Check that your logic app workflow begins with a [Request trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres).
...
1. In the **Choose an operation** search box, enter the name for your logic app custom connector.
   ...
   ![Screenshot shows Power Apps flow editor with a new operation added for custom connector and available actions.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-from-power-apps/power-apps-custom-connector-action.png)
...
1. In the list, find your custom connector, and select the ellipses (**...**) button, and then select **Delete**.
   ...
   ![Screenshot shows Custom connectors page with custom connector management options.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-from-power-apps/delete-custom-connector.png)
...
| Check permissions | Make sure you have the required permissions to access the logic app workflow and environment. You might need specific roles assigned to you. For more information, see the following documentation: <br><br>- [Secure data and access to workflows](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app?tabs=azure-portal#access-to-logic-app-operations) <br><br>- [Access for inbound calls to request-based triggers](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app?tabs=azure-portal#access-for-inbound-calls-to-request-based-triggers) |
