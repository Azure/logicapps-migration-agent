<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-custom-api-authentication -->
...
* [No code changes](#no-code): Protect your API with [Microsoft Entra ID](https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-whatis) through the Azure portal, so you don't have to update your code or redeploy your API.
...
1.  On the toolbar, select **New registration**.
    ...
    ![Screenshot shows Azure portal with Microsoft Entra tenant, App registrations pane, and selected command for New registration.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/new-app-registration-azure-portal.png)
...
1. On the **Register an application** page, follow these steps:
   ...
   1. Under **Redirect URI**, select **Web** as the platform. Next to this option, provide a unique URL for the location to return the authentication response.
      ...
      ![Screenshot shows pane to register an application with application identity name and URL for where to send authentication response.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/logic-app-identity-azure-portal.png)
   ...
   ![Screenshot shows the application identity for your logic app.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/logic-app-identity-created.png)
...
1. On the **Overview** page, under **Essentials**, copy and save the **Application (client) ID** to use as the client ID for your logic app in Part 3.
   ...
   ![Screenshot shows the application (client) ID highlighted.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/logic-app-application-id.png)
...
1. On the **Add a client secret** pane, for **Description**, provide a name for your secret. For **Expires**, select a duration for your secret. When you're done, select **Add**.
   ...
   ![Screenshot shows secret creation for application identity.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/create-logic-app-identity-key-secret-password.png)
   ...
   ![Screenshot shows secret value and secret ID with copy button for secret value selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/logic-app-copy-key-secret-password.png)
...
[Include](https://learn.microsoft.com/en-us/azure/reusable-content/ce-skilling/azure/includes/updated-for-az.md)
...
For more information, learn how to [create a service principal with PowerShell to access resources](https://learn.microsoft.com/en-us/azure/active-directory/develop/howto-authenticate-service-principal-powershell).
...
1. Now, create an application identity for your web app or API app as follows:
   ...
   ![Screenshot shows newly created application identity for web app or API app.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/application-identity-for-web-app.png)
...
1. In the **Identity provider** section, find your previously created application identity. Select the application identity name.
   ...
   ![Screenshot shows Authentication page open for newly created application identity.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/custom-api-app-select-app-identity.png)
...
1. On the **Overview** page, find the values for **Application (client) ID** and **Directory (tenant) ID**. Copy and save the values for use in Part 3.
   ...
   ![Screenshot shows Overview page open for the web app application identity and selected values for the application (client) ID and directory (tenant) ID.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/logic-apps-custom-api-authentication/app-identity-web-app-details.png)
...
To validate the incoming requests from your logic app workflow to your web app or API app, you can use client certificates. To set up your code, learn [how to configure TLS mutual authentication](https://learn.microsoft.com/en-us/azure/app-service/app-service-web-configure-tls-mutual-auth).
...
[!INCLUDE [guidance-authentication-flows](https://learn.microsoft.com/en-us/azure/logic-apps/includes/guidance-authentication-flows)]
...
<!-- Going further, to implement this authentication entirely in your own code, and not use the Azure portal, learn how to [authenticate with on-premises Active Directory in your Azure app](https://learn.microsoft.com/en-us/azure/app-service/overview-authentication-authorization).
...
* [Deploy and call custom APIs from logic app workflows](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-custom-api-host-deploy-call)
