<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/call-azure-functions-from-workflows -->
...
To run code that performs a specific job in your logic app workflow, you don't have to build a complete app or infrastructure. Instead, you can create and call an Azure function. [Azure Functions](https://learn.microsoft.com/en-us/azure/azure-functions/functions-overview) provides serverless computing in the cloud and the capability to perform the following tasks:
...
- [Run code snippets in workflows](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-add-run-inline-code)
- [Create and run .NET Framework code from Standard workflows](https://learn.microsoft.com/en-us/azure/logic-apps/create-run-custom-code-functions)
...
- An [Azure function app resource](https://learn.microsoft.com/en-us/azure/azure-functions/functions-get-started), which contains one or more Azure functions.
...
- The Azure function that you want to call. You can create this function using the following tools:
  ...
  - [Visual Studio Code](https://learn.microsoft.com/en-us/azure/azure-functions/how-to-create-function-vs-code.md?pivot=programming-language-csharp)
  - [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/functionapp/app)
  - [Azure PowerShell](https://learn.microsoft.com/en-us/powershell/module/az.functions)
  - [ARM template](https://learn.microsoft.com/en-us/azure/templates/microsoft.web/sites/functions)
  ...
  - Your function can't use custom routes unless you defined an [OpenAPI definition](https://learn.microsoft.com/en-us/azure/azure-functions/functions-openapi-definition).
...
1. On your function app, set up [Cross-Origin Resource Sharing (CORS)](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) so that all origins are permitted by following these steps:
   ...
   1. Under **Allowed Origins**, add the asterisk (**`*`**) wildcard character, but remove all the other origins in the list, and select **Save**.
      ...
      ![Screenshot shows Azure portal, CORS pane, and wildcard character * entered under Allowed Origins.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/function-cors-origins.png)

### Access property values inside HTTP requests

Webhook-based functions can accept HTTP requests as inputs and pass those requests to other functions. For example, although Azure Logic Apps has [functions that convert DateTime values](https://learn.microsoft.com/en-us/azure/logic-apps/workflow-definition-language-functions-reference), this basic sample JavaScript function shows how you can access a property inside an HTTP request object that's passed to the function and perform operations on that property value. To access properties inside objects, this example uses the [dot (.) operator](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/Property_accessors):
...
1. Enter the key-value pair for your parameter, for example:
   ...
   ![Screenshot shows function information pane with Queries parameter and example key-value inputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/queries-parameter.png)
...
1. In the **Add an action** pane, follow these steps:
   ...
   1. From the function apps list, select your function app, select the function, and then select **Add action**, for example:
      ...
      ![Screenshot shows Consumption workflow with a selected function app and function.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/select-function-app-function-consumption.png)
...
1. After the function's information box appears, follow these steps:
   ...
   1. For **Request Body**, provide your function's input, which must use the format for a JavaScript Object Notation (JSON) object, for example:
      ...
      ![Screenshot shows Consumption workflow and a function with a Request Body example for the context object payload.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/function-request-body-example-consumption.png)
      ...
      ![Screenshot shows Consumption workflow and a function with a complete Request Body example for the context object payload.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/request-body-example-complete.png)
      ...
      ![Screenshot shows Consumption workflow and a Request Body example that casts context object as a string.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/function-request-body-string-cast-example.png)
...
1. In the **Create Connection** pane, follow these steps:
   ...
   ![Screenshot shows Standard workflow designer with selected function app and function.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/select-function-app-function-standard.png)
...
1. After the function's information box appears, follow these steps:
   ...
   1. For **Request Body**, provide your function's input, which must use the format for a JavaScript Object Notation (JSON) object, for example:
      ...
      ![Screenshot shows Standard workflow and a function with a Request Body example for the context object payload.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/function-request-body-example-standard.png)

      Here, the context object isn't cast as a string, so the object's content gets added directly to the JSON payload. Here's the complete example:

      ![Screenshot shows Standard workflow and a function with a complete Request Body example for the context object payload.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/request-body-example-complete.png)

      If you provide a context object other than a JSON token that passes a string, a JSON object, or a JSON array, you get an error. However, you can cast the context object as a string by enclosing the token in quotation marks (**""**), for example, if you wanted to use the **Received Time** token:

      ![Screenshot shows Standard workflow and a Request Body example that casts context object as a string.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/function-request-body-string-cast-example.png)
...
* [Authenticate access with managed identities](https://learn.microsoft.com/en-us/azure/logic-apps/create-managed-service-identity)
* [Add authentication to outbound calls](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-securing-a-logic-app#add-authentication-outbound)
...
1. [Enable and set up your logic app's managed identity](https://learn.microsoft.com/en-us/azure/logic-apps/create-managed-service-identity).
...
1. On the function app resource menu, under **Development tools**, select **Advanced Tools** > **Go**.
   ...
   ![Screenshot shows function app menu with selected options for Advanced Tools and Go.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/open-advanced-tools-kudu.png)

1. After the **Kudu Plus** page opens, on the Kudu website's title bar, from the **Debug Console** menu, select **CMD**.

   ![Screenshot shows Kudu Services page with opened Debug Console menu and selected option named CMD.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/open-debug-console-kudu.png)
...
1. After the next page appears, from the folder list, select **site** > **wwwroot** > *your-function*.
   ...
   ![Screenshot shows folder list with the opened folders for the site, wwwroot, and your function.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/select-site-wwwroot-function-folder.png)

1. Open the **function.json** file for editing.

   ![Screenshot shows the function.json file with selected edit command.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/edit-function-json-file.png)
...
1. In the **bindings** object, check whether the **authLevel** property exists. If the property exists, set the property value to **`anonymous`**. Otherwise, add that property, and set the value.
   ...
   ![Screenshot shows bindings object with authLevel property set to anonymous.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/set-authentication-level-function-app.png)
...
Either run the PowerShell command named [**Get-AzContext**](https://learn.microsoft.com/en-us/powershell/module/az.accounts/get-azcontext), or in the Azure portal, follow these steps:
...
1. Copy and save your tenant ID for later use, for example:
   ...
   ![Screenshot shows Microsoft Entra ID Properties page with tenant ID's copy button selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/tenant-id.png)
...
1. On the logic app menu, under **Settings**, select **Identity**, and then select either **System assigned** or **User assigned**.
   ...
   - **System assigned**
     ...
     ![Screenshot shows Consumption logic app's Identity page with selected tab named System assigned.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/system-identity-consumption.png)

   - **User assigned**

     1. Select the identity:

        ![Screenshot shows Consumption logic app's Identity page with selected tab named User assigned.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/user-identity-consumption.png)

     1. Copy the identity's **Object (principal) ID**:

        ![Screenshot shows Consumption logic app's user-assigned identity Overview page with the object (principal) ID selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/user-identity-object-id.png)
...
1. On the **All applications** page, in the search box, enter the object ID for your managed identity. From the results, find the matching enterprise application, and copy the **Application ID**:
   ...
   ![Screenshot shows Microsoft Entra tenant page named All applications, with enterprise application object ID in search box, and selected matching application ID.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/find-enterprise-application-id.png)
...
1. On the function app menu, under **Settings**, select **Authentication**, and then select **Add identity provider**.
   ...
   ![Screenshot shows function app menu with Authentication page and selected option named Add identity provider.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/add-identity-provider.png)
...
1. Copy the app registration's **App (client) ID** to use later in the Azure Functions action's **Audience** property for your workflow.
   ...
   ![Screenshot shows new identity provider for function app.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/call-azure-functions-from-workflows/identity-provider-application-id.png)

1. Return to the designer and follow the [steps to authenticate access with the managed identity](https://learn.microsoft.com/en-us/azure/logic-apps/create-managed-service-identity#authenticate-access-with-identity) by using the built-in Azure Functions action.

## Next steps

* [Authentication access to Azure resources with managed identities in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/create-managed-service-identity#authenticate-access-with-identity)
