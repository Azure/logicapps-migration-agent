<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/parse-document-chunk-text -->
...
If you use other content sources, such as Azure Blob Storage, SharePoint, OneDrive, File System, FTP, and so on, you can check whether triggers are available for these sources. You can also check whether actions are available to retrieve the content for these sources. For more information, see [Built-in operations](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference/) and [Managed connectors](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors).

1. In the [Azure portal](https://portal.azure.com), open your logic app resource and workflow in the designer.

1. Under the existing trigger and actions, [follow these general steps to add the **Data Operations** action named **Parse a document**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action#add-action) to your workflow.
...
1. After the action information pane opens, on the **Parameters** tab, in the **Document Content** property, specify the content to parse by following these steps:
   ...
   1. After the dynamic content list opens, select the output that you want from a preceding operation.
      ...
      ![Screenshot shows workflow designer, which has an action named Parse a document with opened dynamic content list and selected Body output from HTTP action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/parse-document-chunk-text/select-http-body.png)

      The **Body** output now appears in the **Document Content** box:

      ![Screenshot shows sample workflow with Body output in the action named Parse a document.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/parse-document-chunk-text/parse-document.png)
...
1. Under the **Parse a document** action, [follow these general steps to add the **Data Operations** action named **Chunk text**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action#add-action).
...
1. After you select the strategy, select inside the **Text** box to specify the content for chunking.
   ...
   1. After the dynamic content list opens, select the output that you want from a preceding operation.
      ...
      ![Screenshot shows workflow designer, action named Chunk text with open dynamic content list, and selected output from Parse a document action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/parse-document-chunk-text/select-parsed-result-text.png)

      The **Text** box now shows the **Parsed result action** output:

      ![Screenshot shows sample workflow with selected parsed result text output in the action named Chunk text.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/parse-document-chunk-text/chunk-text.png)
...
| **Text** | <*content-to-chunk*> | Any | The content to chunk. | See [Limits and configuration reference guide](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#character-limits) |
...
![Screenshot shows complete sample workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/parse-document-chunk-text/complete-example.png)
...
- [Integrate Foundry Tools with Standard workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/azure-ai)
