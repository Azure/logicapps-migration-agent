<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/add-agent-action-create-run-workflow -->
...
![](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/azure-ai-foundry-logic-apps-arch.png)
...
- [Consumption logic app workflow - Hosting options](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview#create-and-deploy-to-different-environments)

## Prerequisites

- An Azure account and subscription. If you don't have an Azure subscription, [sign up for a free Azure account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).

- A [Foundry project](https://learn.microsoft.com/en-us/azure/ai-foundry/how-to/create-projects?tabs=ai-studio), which is created with a default [hub](https://learn.microsoft.com/en-us/azure/ai-foundry/how-to/create-azure-ai-resource?tabs=portal).
  ...
  - [Default roles for projects](https://learn.microsoft.com/en-us/azure/ai-foundry/concepts/rbac-azure-ai-foundry#default-roles-for-projects)

  - [Default roles for hubs](https://learn.microsoft.com/en-us/azure/ai-foundry/concepts/rbac-azure-ai-foundry#default-roles-for-the-hub)

- A deployed Azure OpenAI Service [model for your project](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models).

  If you don't have a deployed model, see [Deploy a model](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource?pivots=web-portal#deploy-a-model).

- An [agent in your project](https://learn.microsoft.com/en-us/azure/ai-services/agents/quickstart?context=%2Fazure%2Fai-foundry%2Fcontext%2Fcontext&pivots=ai-foundry-portal).
...
- The workflow starts with the [trigger named **When an HTTP request is received** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-request-trigger).

- The trigger includes a description, which you provide on the trigger information pane in the workflow designer.

- The workflow ends with the [**Response** action](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-a-response-action).
...
![](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/azure-ai-foundry-logic-apps-arch-full.png)
...
- [Managed connectors that run in global, multitenant Azure](https://learn.microsoft.com/en-us/azure/connectors/managed)
- [Managed connectors technical reference for Azure Logic Apps](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors)
- [Built-in operations that run with Azure Logic Apps runtime](https://learn.microsoft.com/en-us/azure/connectors/built-in)
...
|------------|-------------|
| Logic app workflow support | Agent actions currently support only Consumption logic app workflows in multitenant Azure Logic Apps. A Consumption logic app resource can have only one workflow. For more information, see [Hosting options for deployments](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview#create-and-deploy-to-different-environments). <br><br>Agent actions currently don't support Standard logic app workflows in single-tenant Azure Logic Apps, App Service Environments, or hybrid deployments. A Standard logic app can have multiple workflows. For more information, see [Hosting options for deployments](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview#create-and-deploy-to-different-environments). |
...
1. From your project overview, on the portal navigation menu, under **Build and customize**, select **Agents**. On the **Agents** page, under **My agents**, select your agent.
   ...
   ![Screenshot shows Foundry portal, navigation menu with selected Agents option, and a selected agent.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/select-agent.png)

1. In the **Setup** section next to the agents list, scroll down to the **Actions** section, and select **Add**.

   ![Screenshot shows Foundry portal, agent's Setup sidebar, and selected option for Add action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/add-action.png)

1. In the **Add action** window, select **Azure Logic Apps**.

   ![Screenshot shows Foundry window named Add action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/azure-logic-apps.png)
...
1. Under **Select an action**, select a predefined action with one of the following labels:
   ...
   ![Screenshot shows Foundry window named Add Logic App action with selected action named Get Weather forecast.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/get-weather-forecast-action.png)
...
1. In the **Add Logic App action** window, under the **Enter some basic information**, provide the following information:
   ...
   ![Screenshot shows Foundry window details about the action to create based on the previously selected action, such as action name, description, subscription, resource group, and location.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/action-details.png)
...
1. In the **Connection** column, select **Connect** for the related service or data source.
   ...
   ![Screenshot shows Foundry window, Authenticate page, and selected option for Connect to authenticate access for MSN Weather service.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/authenticate-connections.png)
...
1. For a selected Microsoft authored action, complete the following steps:
   ...
   1. Review the statement that you acknowledge and understand the following events that happen after you leave **Resource** page by selecting **Next**:
      ...
      - Connecting to Azure Logic Apps incurs charges in your Azure account.
        ...
        - [Usage metering, billing, and pricing](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-pricing#consumption-multitenant)
   ...
   1. To consent, select the confirmation box, for example:
      ...
      ![Screenshot shows Foundry window with Resource page and selected confirmation box to create a logic app resource.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/resource-create.png)
...
1. In the **Add Logic App action** window, on the **Schema** page, review the following information, and make sure to provide a description about the circumstances for calling the tool:
   ...
   | **Connection for authentication** | The read-only name for the the connection that the agent uses to access Azure, Microsoft, and external resources without having to ask for credentials every time. For more information, see [Connections in Foundry portal](https://learn.microsoft.com/en-us/azure/ai-foundry/concepts/connections). |
   ...
   ![Screenshot shows Foundry window with Schema page and description about the circumstances to invoke the tool.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/finish-create-action.png)
...
1. When you're ready, select **Create**.
   ...
   ![Screenshot shows Foundry window with Agents page, selected agent, and Setup section with Actions section, showing tool that runs your new action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/add-action-finish.png)
...
1. On the **Agents playground** page, in the user query chat box, ask a question about the weather, for example:
   ...
   **What is the weather in London? Show the results in bullet list format.**
   ...
   ![Screenshot shows Foundry window with Agents playground page, test prompt about London weather with format instructions, and response.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/test-action.png)
...
1. In the results list, under **Resources**, select the logic app resource.
   ...
   ![Screenshot shows Azure portal, title bar search box with logic app resource name, and selected result with logic app workflow name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/find-logic-app-azure-portal.png)
...
1. In the **Run history** list, select the latest workflow run, for example:
   ...
   ![Screenshot shows Azure portal, Run history page, and selected most recent workflow run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/select-workflow-run.png)
...
1. After the monitoring view opens and shows the status for each operation in the workflow, select an operation to open the information pane and review the operation's inputs and outputs.
   ...
   ![Screenshot shows Azure portal, monitoring view for workflow run, selected operation, and information pane with operation inputs and outputs.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/view-inputs-outputs.png)

   For more information about workflow run history, see [View workflow status and run history](https://learn.microsoft.com/en-us/azure/logic-apps/view-workflow-status-run-history?tabs=consumption).
...
1. On the logic app menu, under **Development Tools**, select the designer.
   ...
   ![Screenshot shows Azure portal, workflow designer, and workflow definition created by the agent action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/open-workflow-designer.png)

1. To view an operation's parameters and settings, on the designer, select the operation, for example:

   ![Screenshot shows Azure portal, workflow designer, selected operation, and information pane with operation parameters and other settings.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/add-agent-action-create-run-workflow/view-parameters.png)
...
1. To expand the workflow's behavior, you can add more actions by following the steps in [Build a workflow with a trigger or action](create-workflow-with-trigger-or-action.md?tabs=consumption#add-action).
   ...
   - The workflow must start with the [**When an HTTP request is received** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-request-trigger).

   - The trigger requires a description, which you can find on the trigger information pane in the designer.

   - The workflow must end with the [**Response** action](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-a-response-action).
...
- [Plan and manage costs for Foundry](https://learn.microsoft.com/en-us/azure/ai-foundry/how-to/costs-plan-manage)
