<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/create-model-context-protocol-server-standard -->
...
![Diagram that shows agent or model interactions with MCP client and MCP server components.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/mcp-server-architecture.png)
...
- [What is an AI Agent?](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/overview#what-is-an-ai-agent)
- [About LLMs](https://learn.microsoft.com/en-us/training/modules/introduction-large-language-models/2-understand-large-language-models)
...
- One or more workflows in your logic app to use as tools for your MCP server. These workflows must meet the following requirements:
  ...
  - [Create a Standard logic app and workflow using the Azure portal](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal)
...
- No other requirements exist to use the Streamable HTTP transport. However, to use the Server-Sent Events (SSE) transport, your logic app must meet the following requirements:
  ...
  - Your logic app requires virtual network integration. See [Secure traffic between Standard logic apps and Azure virtual networks using private endpoints](https://learn.microsoft.com/en-us/azure/logic-apps/secure-single-tenant-workflow-virtual-network-private-endpoint).
...
![Screenshot shows trigger information pane with description box and example description.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/trigger-description.png)
...
![Screenshot shows trigger information pane with Request Body Json Schema box and example descriptions for input parameters.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/input-parameter-descriptions.png)
...
1. In the trigger information pane, under **Request Body JSON Schema**, enter a schema for the expected request content payload.
  ...
  - You can set up error handling and use the `runAfter` property to return the appropriate error message to the caller. For more information, see [Manage the "run after" behavior](https://learn.microsoft.com/en-us/azure/logic-apps/error-exception-handling#manage-the-run-after-behavior).
...
1. On the logic app sidebar, under **Agents**, select **MCP servers** to open the **MCP servers** page, for example:
   ...
   ![Screenshot shows the Azure portal, Standard logic app resource, and MCP servers page.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/mcp-servers.png)
...
1. Under **Tools**, follow these steps to select a connector and the actions to create as workflows.
      ...
      1. In the **Connectors** section, select **Add**.
         ...
         ![Screenshot shows the Connectors section with the selected option for Add.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/add-connector.png)

      1. On the **Add connector** pane and the **Choose connector** tab, find and select the connector to use, for example:

         ![Screenshot shows Add connector pane with selected Office 365 Outlook connector.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/choose-connector.png)

      1. On the **Select actions** tab, select each action that you want to create as a tool, for example:

         ![Screenshot shows Add connector pane with selected connector actions to create as tools.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/select-actions.png)
      ...
      ![Screenshot shows the Connectors and Actions sections with the added actions as tools.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/tools-list.png)
  ...
  1. To help an agent or LLM choose the correct tool and pass correctly sourced inputs to tool parameters, review and update each tool's setup by following these steps:
     ...
     1. On the **Edit: <*tool-name*>** pane, provide the following information:
        ...
        | Section | Description |
        |---------|-------------|
        | **Description** | Describes the purpose for the action-backed tool to help an agent or LLM determine when to use the tool. A default description exists, but you can customize the text for your needs. <br><br>The default text comes from the [connector's API Swagger description](https://learn.microsoft.com/en-us/connectors/connector-reference/connector-reference-logicapps-connectors), for example, [Actions - Office 365 Outlook](https://learn.microsoft.com/en-us/connectors/office365/). |
        ...
        ![Screenshot shows Edit pane for an example tool.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/tool-parameters.png)
...
1. In Visual Studio Code, from the **View** menu, select **Command Palette**. Find and select **MCP: Add Server**.
   ...
   ![Screenshot shows Visual Studio Code, Command Palette, and command to add MCP server.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/visual-studio-code-mcp-add-server.png)
...
1. In the *mcp.json* file, select the **Start** or **Restart** link to establish connectivity for your MCP server, for example:
   ...
   **OAuth**
   ...
   ![Screenshot shows mcp.json file for OAuth with Start link selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/start-server-mcp-json-file-oauth.png)

   **Key-based**

   ![Screenshot shows mcp.json file for Key-based with Start link selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/start-server-mcp-json-file-key-based.png)
...
1. Sign in and give consent to call your MCP server.
   ...
   **OAuth**
   ...
   ![Screenshot shows mcp.json file for OAuth with Running status selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/running-mcp-json-file-oauth.png)

   **Key-based**

   ![Screenshot shows mcp.json file for Key-based with Running status selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-model-context-protocol-server-standard/running-mcp-json-file-key-based.png)
...
[!INCLUDE [ai-action-parameter-values-runtime](https://learn.microsoft.com/en-us/azure/logic-apps/includes/ai-action-parameter-values-runtime)]

## Related content

- [Workflows with AI agents and models in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/agent-workflows-concepts)
- [Create workflows that use AI agents and models](https://learn.microsoft.com/en-us/azure/logic-apps/create-agent-workflows)
- [Run Consumption workflows as actions for agents in Microsoft Foundry](https://learn.microsoft.com/en-us/azure/logic-apps/add-agent-action-create-run-workflow)
