<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/connectors/code-interpreter-python-container-apps-session -->
...
![Screenshot shows workflow with agent and tool, which uses Code Interpreter actions that upload files to Container Apps session pool and runs Python code for file processing.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/code-interpreter-python-container-apps-session/overview.png)
...
- A container app and a code interpreter session pool in Azure Container Apps.
  ...
  - To create the app, see [Quickstart: Deploy your first container app using the Azure portal](https://learn.microsoft.com/en-us/azure/../container-apps/quickstart-portal). To make sure that your data never leaves your network, turn on network isolation on your container app.
  ...
  - To create the session pool, see [Create session pools in Azure Container Apps](https://learn.microsoft.com/en-us/azure/../container-apps/session-pool). You can use either the Azure portal or Azure CLI.

    > [!IMPORTANT]
    >
    > For the session pool type, make sure that you select **Python code interpreter**. 

  - Optional: You can upload the file to the session by using the Azure portal or sending an HTTPS request. You can then reference the file as a data source in Python code. For more information, see [Upload a file - Serverless code interpreter sessions in Azure Container Apps](https://learn.microsoft.com/en-us/azure/../container-apps/sessions-code-interpreter.md#upload-a-file). Otherwise, you can use the **Upload file** action from the **Code Interpreter (Python Container Apps session)** connector.
    ...
    > For this task, see [Give identity access to resources](https://learn.microsoft.com/en-us/azure/authenticate-with-managed-identity.md?tabs=standard#give-identity-access-to-resources).
    ...
    1. On the **Playground** toolbar, select **Manage files**.
       ...
       ![Screenshot shows a Container Apps session pool resource, sidebar menu with Playground selected, and Playground toolbar with the Manage files command selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/code-interpreter-python-container-apps-session/manage-files-session-pool.png)
...
- A Standard logic app resource and an agent workflow. Make sure that you set up a connection between the **Default Agent** action and an AI model to use for code generation.
  ...
  To create this workflow, see [Create workflows that use AI agents and models to complete tasks in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/create-agent-workflows).
...
For technical information about this connector's operations, see the [connector's reference documentation](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference/acasession/).
...
1. In the **Default Agent** action, follow the [general steps to create a tool and add the **Code Interpreter (Python Container Apps session)** action named **Upload file**](https://learn.microsoft.com/en-us/azure/create-agent-workflows.md#create-a-tool-to-get-weather-forecast).
...
1. In the action information pane, expand **Input files to upload**, and provide the following values:
   ...
   ![Screenshot shows agent workflow with HTTP action that gets a file and Upload file action that adds file to session pool.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/code-interpreter-python-container-apps-session/upload-file-parameters.png)
...
1. For the **Default Agent** action, rename the agent for your scenario, and follow these steps:
   ...
   1. Based on whether you created a tool with the **Upload file** action from the preceding section, choose either option:
      ...
      - If a tool with the **Upload file** action exists, directly under that action, follow the [general steps to add the **Code Interpreter (Python Container Apps session)** action named **Execute Python code**](https://learn.microsoft.com/en-us/azure/add-trigger-action-workflow.md?tabs=standard#add-action) to the tool.

      - If your workflow uploads the file in another way, follow the [general steps to create a tool by using the **Code Interpreter (Python Container Apps session)** action named **Execute Python code**](https://learn.microsoft.com/en-us/azure/create-agent-workflows.md#create-a-tool-to-get-weather-forecast).

   1. If you're prompted to create a connection, on the **Create connection** pane, provide the [connection information](#connection-information) for the session pool.

   1. On the new tool, follow the [general steps to create an agent parameter for model-generated outputs](https://learn.microsoft.com/en-us/azure/create-agent-workflows.md#create-agent-parameters-for-the-get-forecast-action).
      ...
      ![Screenshot shows agent workflow and code reference inside action for Execute Python Code.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/code-interpreter-python-container-apps-session/python-code-reference.png)
   ...
   1. For the **Execute Python code** action to use the file from the **Upload file** action, set the **Session ID** parameter value to the session ID for the **Upload file** action by following these steps:
      ...
      ![Screenshot shows advanced parameters and session ID parameter for Execute Python Code action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/connectors/media/code-interpreter-python-container-apps-session/session-id.png)
...
- [AI agent workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/agent-workflows-concepts)
