<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/set-up-on-behalf-of-user-flow -->
...
![Screenshot shows workflow designer, example Get emails (V3) action, and connection pane with selected option for Create as per-user connection.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/create-per-user-connection.png)
...
- A Standard logic app resource and conversational agentic workflow
  ...
  ![Screenshot shows Azure portal, Standard logic app with conversational agentic workflow, and empty Admin agent.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/admin-agent.png)
...
- To test the single user scenario in this guide, you only need your Azure account credentials. Testing this scenario happens through the internal chat interface integrated with your agentic workflow in the Azure portal.
  ...
  - To make the external chat client available, you must set up your logic app resource with [Easy Auth (App Service Authentication)](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-authentication-agent-workflows). This authentication and authorization flow also requires people to sign in and confirms their identity and permissions before they can access and chat with the agent tool.
  ...
  For more information about authentication and authorization for agentic workflows, see [Authentication and authorization in AI agentic workflows](https://learn.microsoft.com/en-us/azure/logic-apps/agent-workflows-concepts#authentication-and-authorization).
...
1. On the **Create connection** pane, select **Create as per-user connection?**
   ...
   ![Screenshot shows workflow designer, Get emails (V3) action, and connection pane with selected option for Create as per-user connection.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/create-per-user-connection.png)
...
1. Provide any required information for the connector action.
   ...
   ![Screenshot shows workflow designer, Get emails (V3) action selected, and action information pane with setup details.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/get-emails-action.png)
...
1. On the designer, select the tool's title bar to open the tool's information pane. Complete the following tasks if you're following along with the example:
   ...
   1. In the **Description** box, enter a concise but useful tool description that describes the purpose and guidance about the data that the tool works on.
      ...
      ![Screenshot shows workflow designer, Tool action selected, and tool information pane with setup details.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/tool-setup.png)
...
1. In the chat interface, ask a question to test the agent tool action.
   ...
   ![Screenshot shows internal chat interface with test question and authentication prompt.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/chat-sign-in-prompt.png)

1. Sign in with your credentials.

   The chat interface now shows that authentication successfully completed, for example:

   ![Screenshot shows internal chat interface with successful authentication message.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/set-up-on-behalf-of-user-flow/chat-authentication-success.png)
...
1. Follow the [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-authentication-agent-workflows#external-chat-client) to open the external chat client  outside the Azure portal, 
...
| **429 Too many requests** | Also known as *throttling* or *rate limiting*. Add an exponential interval between request retries or ask the user to narrow the request or get more specific. For more information, see [Handle errors and exceptions in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/error-exception-handling#retry-policies). |
...
- [Set up Easy Auth (App Service Authentication) for agentic workflows](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-authentication-agent-workflows)
- [Authentication and authorization in AI agentic workflows](https://learn.microsoft.com/en-us/azure/logic-apps/agent-workflows-concepts#authentication-and-authorization)
