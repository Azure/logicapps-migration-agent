<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-gateway-connection -->
...
In Consumption logic app workflows, you can connect to on-premises data sources only by using [connectors that provide access through the on-premises data gateway](../connectors/managed#on-premises-connectors).

In Standard logic app workflows, you can directly access on-premises data sources and resources in Azure virtual networks by using [built-in service provider connectors](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference/), which don't require the on-premises data gateway. Instead, you provide information that authenticates your identity and authorizes access to your data source. If a built-in service provider connector isn't available for your data source, but a managed connector is available, you have to use the on-premises data gateway.

This guide shows how to create the Azure data gateway resource after you [install the on-premises gateway on your local computer](logic-apps-gateway-install).

For more information, see the following documentation:

- [Connectors that can access on-premises data sources](../connectors/managed#on-premises-connectors)
- [How the gateway works](logic-apps-gateway-install#gateway-cloud-service)

For information about how to use a gateway with other services, see the following documentation:

- [Microsoft Power Automate on-premises data gateway](https://learn.microsoft.com/en-us/power-automate/gateway-reference)
- [Microsoft Power BI on-premises data gateway](https://learn.microsoft.com/en-us/power-bi/service-gateway-onprem)
- [Microsoft Power Apps on-premises data gateway](https://learn.microsoft.com/en-us/powerapps/maker/canvas-apps/gateway-reference)
- [Azure Analysis Services on-premises data gateway](../analysis-services/analysis-services-gateway)

## Supported data sources

In Azure Logic Apps, the on-premises data gateway supports [on-premises connectors](../connectors/managed#on-premises-connectors) for the following data sources:

- [Apache Impala](https://learn.microsoft.com/en-us/connectors/impala)
- [BizTalk Server](https://learn.microsoft.com/en-us/connectors/biztalk)
- [File System](https://learn.microsoft.com/en-us/connectors/filesystem)
...
- [IBM Informix](https://learn.microsoft.com/en-us/connectors/informix)
- [IBM MQ](https://learn.microsoft.com/en-us/connectors/mq)
- [MySQL](https://learn.microsoft.com/en-us/connectors/mysql)
- [Oracle Database](https://learn.microsoft.com/en-us/connectors/oracle)
- [PostgreSQL](https://learn.microsoft.com/en-us/connectors/postgresql)
- [SAP](https://learn.microsoft.com/en-us/connectors/sap)
- [SharePoint Server](https://learn.microsoft.com/en-us/connectors/sharepointonline)
- [SQL Server](https://learn.microsoft.com/en-us/connectors/sql)
- [Teradata](https://learn.microsoft.com/en-us/connectors/teradata)

You can also create [custom connectors](custom-connector-overview) that connect to data sources over HTTP or HTTPS by using REST or SOAP. Although a gateway itself doesn't incur extra costs, the [Azure Logic Apps pricing model](logic-apps-pricing) applies to connectors and other Azure Logic Apps operations.

## Limitations

Azure Logic Apps supports read and write operations through the data gateway, but these operations have [limits on their payload size](https://learn.microsoft.com/en-us/data-integration/gateway/service-gateway-onprem#considerations).

## Prerequisites

You already [installed an on-premises data gateway on a local computer](logic-apps-gateway-install). This data gateway installation must exist before you can create a data gateway resource that links to this installation. You can install only one data gateway per local computer.

You have the [same Azure account and subscription](logic-apps-gateway-install#requirements) that you used for your gateway installation. This Azure account must belong only to a single [Microsoft Entra tenant or directory](../active-directory/fundamentals/active-directory-whatis#terminology). You have to use the same Azure account and subscription to create your gateway resource in Azure because only the gateway administrator can create the gateway resource in Azure. Service principals currently aren't supported.
...
Sign in to the [Azure portal](https://portal.azure.com) with the same Azure account that you used to install the gateway.

In the Azure portal search box, enter **on-premises data gateway**, and then select **On-premises data gateways**.

![Screenshot shows Azure portal search box with the words, on-premises data gateway. The results list shows the selected option, On-premises data gateways.](https://learn.microsoft.commedia/connect-on-premises-data-sources/search-for-on-premises-data-gateway.png)

Under **On-premises data gateways**, select **Create**.

![Screenshot shows the page for On-premises data gateways with the selected option for Create.](https://learn.microsoft.commedia/connect-on-premises-data-sources/add-azure-data-gateway-resource.png)

Under **Create a gateway**, provide the following information for your gateway resource. When you're done, select **Review + create**.

Property
Description

**Subscription**
Select the Azure subscription for the Azure account that you used for the gateway installation. The default subscription is based on the Azure account that you used to sign in.

**Resource group**
Select the [Azure resource group](../azure-resource-manager/management/overview) that you want to use.

**Name**
Enter a name for your gateway resource that contains only letters, numbers, hyphens (`-`), underscores (`_`), parentheses (`(`, `)`), or periods (`.`).

**Region**
Select the same region or location that you selected for the gateway cloud service during [gateway installation](logic-apps-gateway-install). Otherwise, your gateway installation doesn't appear in the **Installation Name** list. Your logic app resource location can differ from your gateway resource location.

**Installation Name**
Select a gateway installation, which appears in the list only when these conditions are met: 
...
- Your Azure account belongs to a single [Microsoft Entra tenant or directory](../active-directory/fundamentals/active-directory-whatis#terminology) and is the same account that you used for the gateway installation. 

For more information, see [Frequently asked questions](#frequently-asked-questions).
...
![Screenshot shows the page for Create a gateway. The Name, Region, and other boxes contain values. The button, Review + create, appears selected.](https://learn.microsoft.commedia/connect-on-premises-data-sources/on-premises-data-gateway-create-connection.png)
...
In Consumption logic app workflows, if a connector has a [managed version](../connectors/managed#on-premises-connectors)
and a [built-in version](../connectors/built-in), use the managed version, which includes the gateway selection capability.
...
![Screenshot shows SQL Server managed connector with values in the Connection Name, Authentication Type, and other parameter boxes.](https://learn.microsoft.commedia/connect-on-premises-data-sources/logic-app-gateway-connection.png)
...
To find all the API connections for your Consumption logic app resource, on your logic app menu, under **Development Tools**, select **API connections**.

![Screenshot shows Azure portal, Consumption logic app resource, and resource menu. On resource menu, under Development Tools, API connections is selected.](https://learn.microsoft.commedia/connect-on-premises-data-sources/logic-app-api-connections.png)

Select the connection that you want to edit. On the connection pane, go to the connection menu, under **General**, select **Edit API connection**.
...
[stopping and restarting the gateway Windows service account](logic-apps-gateway-install#restart-gateway)
...
To find all the API connections for your Standard logic app resource, on your logic app menu, under **Workflows**, select **Connections**.

On the **Connections** page, select **API Connections**.

On the **API Connections** tab, select the connection that you want to edit.

On the connection page, go to the connection menu, under **General**, select **Edit API connection**.
...
On the gateway resource toolbar, select **Delete**.

![Screenshot shows on-premises data gateway resource in the Azure portal. On the toolbar, Delete is selected.](https://learn.microsoft.commedia/connect-on-premises-data-sources/delete-on-premises-data-gateway.png)
...
Your Azure account doesn't belong to only a single [Microsoft Entra tenant or directory](../active-directory/fundamentals/active-directory-whatis#terminology). Check that you're using the same Microsoft Entra tenant or directory that you used during gateway installation.
...
- [Secure your logic apps](logic-apps-securing-a-logic-app)
- [Common examples and scenarios for logic apps](logic-apps-examples-and-scenarios)
