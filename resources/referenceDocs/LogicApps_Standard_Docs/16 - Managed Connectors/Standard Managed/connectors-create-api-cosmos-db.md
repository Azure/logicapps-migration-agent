<!-- Source: https://learn.microsoft.com/en-us/azure/connectors/connectors-create-api-cosmos-db -->
...
You can connect to Azure Cosmos DB from both Consumption and Standard logic app workflows by using the [*managed connector* operations](https://learn.microsoft.com/en-us/connectors/documentdb/), which are hosted, managed, and run in global, multitenant Azure. For Standard workflows, Azure Cosmos DB also provides [*built-in* operations](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference/azurecosmosdb/) that run alongside the runtime for single-tenant Azure Logic Apps. Built-in operations offer better performance, higher throughput, and sometimes different functionality. For example, in a Standard workflow, you can use the built-in trigger to monitor an Azure Cosmos DB container for new or updated items. You can combine Azure Cosmos DB operations with others in a workflow to support scenarios like event sourcing and general data processing.
...
- The Azure Cosmos DB connector supports only Azure Cosmos DB accounts created with [Azure Cosmos DB for NoSQL](https://learn.microsoft.com/en-us/azure/cosmos-db/choose-api#coresql-api).

## Prerequisites

- An Azure account and subscription. If you don't have an Azure subscription, [sign up for a free Azure account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).

- An [Azure Cosmos DB account](https://learn.microsoft.com/en-us/azure/cosmos-db/sql/create-cosmosdb-resources-portal).
...
- For reference information about the Azure Cosmos DB *managed connector* operations, such as triggers, actions, and limits, see the [managed connector's reference page](https://learn.microsoft.com/en-us/connectors/documentdb/).

- For reference information about the Azure Cosmos DB *built-in* operations, such as triggers, actions, and limits, see the [built-in operations reference page](https://learn.microsoft.com/en-us/azure/logic-apps/connectors/built-in/reference/azurecosmosdb/).

## Add Azure Cosmos DB trigger

In Azure Logic Apps, every workflow must start with a [trigger](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview.md#logic-app-concepts), which fires when a specific event happens or when a specific condition is met.

If you're working with a Standard workflow, the built-in trigger named **When an item is created or modified** is available and is based on the [Azure Cosmos DB change feed design pattern](https://learn.microsoft.com/en-us/azure/cosmos-db/change-feed-design-patterns). This trigger is unavailable for Consumption workflows.
...
1. Follow [these general steps to add the trigger named **When an item is created or modified**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-trigger).
...
1. On the trigger information pane, on the **Parameters** tab, provide the following necessary information:
   ...
   | **Create Lease Container** | No | **No** or **Yes** | If the lease container exists in the specified database, select **No**. To create this container, select **Yes**. If you select **Yes** and are using manual throughput dedicated for each container, make sure to open the **Advanced parameters** list to select the **Lease Container Throughput** parameter. Enter the number of [request units (RUs)](https://learn.microsoft.com/en-us/azure/cosmos-db/request-units) to provision for this container. |
   ...
   ![Screenshot shows Azure portal, designer for Standard workflow, and trigger named When an item is created or modified.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/trigger-standard.png)
...
In Azure Logic Apps, an [action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview.md#logic-app-concepts) is a step in a workflow that follows a trigger or another action. The Azure Cosmos DB connector offers actions for both Consumption and Standard workflows. The following examples show how to use an action that creates or updates a document.
...
1. If the workflow is blank, add any trigger that you want.
   ...
   This example starts with the [**When a HTTP request is received** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-request-trigger).

1. Under the trigger or action where you want to add the Azure Cosmos DB action, follow [these general steps to add the **Azure Cosmos DB** action that you want](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=consumption#add-action).
...
1. In the action information pane, on the **Parameters** tab, provide the following necessary information:
   ...
   ![Screenshot shows Azure portal, designer for Consumption workflow, and action named Create or update document (V3).](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/action-consumption.png)
...
1. If the workflow is blank, add any trigger that you want.
   ...
   This example starts with the [**When a HTTP request is received** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-reqres#add-request-trigger), which uses a basic schema definition to represent the item that you want to create:

   ![Screenshot shows Azure portal, designer for Standard workflow, and trigger named When a HTTP request is received.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/http-trigger-standard.png)

1. Under the trigger or action where you want to add the Azure Cosmos DB action, follow [these general steps to add the **Azure Cosmos DB** action that you want](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action).
...
1. In the action information pane, on the **Parameters** tab, provide the following necessary information:
   ...
   ![Screenshot shows Azure portal, designer for Standard workflow, and Azure Cosmos DB built-in action named Create or update item.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/create-action-standard.png)
...
1. If the workflow is blank, add any trigger that you want.
   ...
   This example starts with the [**Recurrence** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence).

1. Under the trigger or action where you want to add the Azure Cosmos DB action, follow [these general steps to add the **Azure Cosmos DB** action named **Query items**](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action).
...
1. In the action information pane, on the **Parameters** tab, provide the following necessary information:
   ...
   ![Screenshot shows Azure portal, designer for Standard workflow, and Azure Cosmos DB built-in action named Query items.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/query-action-standard.png)

1. Configure any other parameters or settings as needed.

1. Under the **Query items** action, follow [these general steps to add an action that you want to run on all the returned query items](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action?tabs=standard#add-action).
...
1. In the **Delete an item** action, you can access outputs from the **Query items** action by following these steps:
   ...
   1. From the **Query items** section in the list, select the output you want, or select **See more** for more outputs.
      ...
      ![Screenshot shows Azure portal, designer for Standard workflow, and Azure Cosmos DB built-in action named Delete an item.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-cosmos-db/delete-item-standard.png)
