<!-- Source: https://learn.microsoft.com/en-us/azure/connectors/connectors-create-api-azure-event-hubs -->
...
For information about this connector's operations, their parameters, and other technical information, such as limits, known issues, and so on, see the [Event Hubs connector's reference page](https://learn.microsoft.com/en-us/connectors/eventhubs/).

## Prerequisites

- An Azure account and subscription. If you don't have a subscription, [sign up for a free Azure account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).

- An [Event Hubs namespace and event hub](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-create).
  ...
  1. If you plan to later select an authentication type that requires the connection string for your Event Hubs namespace, save that string now so that you can enter that information later:
     ...
     1. Find your primary key's connection string. Copy and save the connection string for later use.
        ...
        ![Screenshot shows primary key connection string with copy button selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-azure-event-hubs/find-event-hub-namespace-connection-string.png)
...
In Azure Logic Apps, every workflow must start with a [trigger](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview.md#logic-app-concepts), which fires when a specific condition is met. Each time the trigger fires, Azure Logic Apps creates an instance of your workflow and starts to run the steps in the workflow.
...
1. On the designer, follow these [general steps to add the **Event Hubs** trigger you want](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow.md#add-trigger) to your workflow.

   For more information, see [Event Hubs - Triggers](https://learn.microsoft.com/en-us/connectors/eventhubs/#triggers). This example continues with the trigger named **When events are available in Event Hub**. This trigger checks for new events in your event hub and starts a workflow run when new events exist.
...
1. In the trigger, provide the necessary information for your selected trigger.
   ...
   | **Time zone** | No | Applies only when you specify a start time because this trigger doesn't accept UTC offset. Select the time zone that you want to apply. <br><br>For more information, see [Schedule and run recurring workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence). |
   | **Start time** | No | Provide a start time in this format: <br><br>YYYY-MM-DDThh:mm:ss if you select a time zone<br><br>-or-<br><br>YYYY-MM-DDThh:mm:ssZ if you don't select a time zone<p>For more information, see [Schedule and run recurring workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence). |
   ...
   | **Consumer group name** | No | The name for the [Event Hubs consumer group](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-features.md#consumer-groups) to use for reading events. If unspecified, the default consumer group is used. |
   | **Minimum partition key** | No | Enter the minimum [partition](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-features.md#partitions) ID to read. By default, all partitions are read. |
   | **Maximum partition key** | No | Enter the maximum [partition](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-features.md#partitions) ID to read. By default, all partitions are read. |
...
In Azure Logic Apps, an [action](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview.md#logic-app-concepts) follows the trigger or another action and performs some task in your workflow. The following steps describe the general way to add an **Event Hubs** action such as **Send event**. For this example, the workflow starts with an **Event Hubs** trigger that checks for new events in the event hub.
...
1. On the designer, follow these [general steps to add the **Event Hubs** action you want](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow.md#add-action) to your workflow.

   For more information, see [Event Hubs - Actions](https://learn.microsoft.com/en-us/connectors/eventhubs/#actions). This example continues with the action named **Send event**.
...
1. In the action, provide the necessary information for your selected action.
   ...
   | Parameter | Required | Description |
   |-----------|----------|-------------|
   | **Partition key** | No | The [partition](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-features.md#partitions) ID for where to send the event. |
   ...
   ![Screenshot shows Event Hubs action with the advanced parameter Content added to the action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/connectors/media/connectors-create-api-azure-event-hubs/event-hubs-send-event-action-example.png)
