<!-- Source: https://learn.microsoft.com/en-us/azure/event-grid/monitor-virtual-machine-changes-logic-app -->
...
In this tutorial, you create a Consumption logic app resource that runs in [*multitenant* Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview). The app is based on the [Consumption pricing model](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-pricing.md#consumption-pricing). Using this logic app resource, you create a workflow that monitors changes to a virtual machine, and sends emails about those changes. When you create a workflow that has an event subscription to an Azure resource, events flow from that resource through Azure Event Grid to the workflow. 

![Screenshot shows the workflow designer with a workflow that monitors a virtual machine using Azure Event Grid.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/monitor-virtual-machine-logic-app-overview.png)
...
- An email account from an email service that works with Azure Logic Apps for sending notifications, such as Office 365 Outlook, Outlook.com, or Gmail. For other providers, see [the connectors list](/connectors/).
  ...
  > If you have a Gmail consumer account, you can use this connector with only specific Google-approved services, or you can [create a Google client app to use for authentication with your Gmail connector](https://learn.microsoft.com/en-us/connectors/gmail/#authentication-and-bring-your-own-application). 
...
- A [virtual machine](https://azure.microsoft.com/services/virtual-machines) that's alone in its own Azure resource group. If you need a virtual machine, see [Create a Windows virtual machine in the Azure portal](https://learn.microsoft.com/en-us/azure/virtual-machines/windows/quick-create-portal). To make the virtual machine publish events, you don't need to do anything else.

- If you have a firewall that limits traffic to specific IP addresses, set up your firewall to allow access for Azure Logic Apps to communicate through the firewall. You need to allow access for both the [inbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config.md#inbound) and [outbound](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config.md#outbound) IP addresses used by Azure Logic Apps in the Azure region where you create your logic app.

  This example uses managed connectors that require your firewall to allow access for *all* the [managed connector outbound IP addresses](https://learn.microsoft.com/en-us/connectors/common/outbound-ip-addresses) in the Azure region for your logic app resource.
...
1. From the Azure home page, select **Create a resource** > **Integration** > **Logic App**.
   ...
   ![Screenshot shows the Azure portal with options to create a logic app resource.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/azure-portal-create-logic-app.png)

1. Select **Consumption** > **Multi-tenant**.

1. Under **Create Logic App**, provide information about your logic app resource:

   ![Screenshot shows the logic apps creation menu, showing details like name, subscription, resource group, and location.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/create-logic-app.png)
   ...
   > For more information, see [Differences between Standard single-tenant logic apps versus Consumption multitenant logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/single-tenant-overview-compare).
...
1. On the designer, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow.md?tabs=consumption#add-a-trigger-to-start-your-workflow) to add the Azure Event Grid trigger named **When a resource event occurs**to your workflow.
...
1. On the **Create connection** pane, provide the following information:
   ...
   ![Screenshot shows the workflow designer with the sign-in prompt for Azure Event Grid.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/sign-in.png)
...
1. Now, set up your workflow with a subscription to events from the publisher.
   ...
   ![Screenshot shows the workflow designer with the trigger details editor open.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/logic-app-trigger-details.png)

   | Parameter | Required | Value | Description |
   | -------- | -------- | ----- | ----------- |
   | **Resource Type** | Yes | <*event-publisher-Azure-resource-type*> | Select the Azure resource type for the event publisher. For more information, see [Azure resource providers and types](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types). For this tutorial, select the **Microsoft.Resources.ResourceGroups** to monitor Azure resource groups. |
   ...
   | **Event Type Item** | No | <*event-types*> | Select one or more specific event types to filter and send to Azure Event Grid. For example, you can optionally add these event types to detect when resources are changed or deleted: <br><br>- **Microsoft.Resources.ResourceActionSuccess** <br>- **Microsoft.Resources.ResourceDeleteSuccess** <br>- **Microsoft.Resources.ResourceWriteSuccess** <br><br>For more information, see: <br><br>- [Azure resource group as an Event Grid source](https://learn.microsoft.com/en-us/azure/event-grid/event-schema-resource-groups) <br>- [Understand event filtering](https://learn.microsoft.com/en-us/azure/event-grid/event-filtering) <br>- [Filter events for Event Grid](https://learn.microsoft.com/en-us/azure/event-grid/how-to-filter-events) |
...
1. On the designer, follow these [general steps](https://learn.microsoft.com/en-us/azure/logic-apps/add-trigger-action-workflow.md#add-action) to add the built-in action named **Condition** to your workflow.
   ...
   ![Screenshot shows the workflow designer with an empty condition added to the workflow.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/empty-condition.png)
...
1. To rename the condition, on the action pane, in the action title, select **Condition**. Rename the title to **If a virtual machine in your resource group has changed**.
   ...
   ![Screenshot shows the workflow designer and the action title box with the new name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/rename-condition.png)

1. Create a condition that checks the event `body` for a `data` object where the `operationName` parameter is equal to the **Microsoft.Compute/virtualMachines/write** operation. For more information, see [Azure Event Grid event schema](https://learn.microsoft.com/en-us/azure/event-grid/event-schema).
   ...
   1. On the first row under **And**, select inside the left box to show the input options, which are the dynamic content list (lightning icon) and expression editor (function icon). Select the expression editor.
      ...
      ![Screenshot shows the workflow designer with the renamed Condition action and expression editor open.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/condition-choose-expression.png)
   ...
   1. In the editor, make sure **Function** is selected. In the editor box, enter the following expression, which returns the operation name from the trigger, and then select **Add**:
      ...
      ![Screenshot shows workflow designer and condition editor with expression to extract the operation name.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/condition-add-data-operation-name.png)
   ...
   ![Screenshot shows the workflow designer with a condition that compares the operation.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/complete-condition.png)
   ...
   ![Screenshot shows the workflow designer with a condition that resolved tokens.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/resolved-condition.png)

1. Save your workflow.

## Send email notifications

Now add an [*action*](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-overview.md#logic-app-concepts) so that you can receive an email when the specified condition is true.

1. In the condition's **True** box, select **+** > **Add an action**.

   ![Screenshot shows the workflow designer with the condition's True pane open and selected option for Add an action.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/condition-true-add-action.png)
...
1. In the action pane, provide the following information about the email to send:
   ...
   ![Screenshot shows the workflow designer with selected outputs to send in email when the virtual machine is updated.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/logic-app-send-email-details.png)

   Your finished workflow now looks like the following example:

   ![Screenshot shows designer with complete workflow and details for trigger and actions.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/logic-app-completed.png)
...
1. To check that your workflow is getting the specified events, update your virtual machine.
   ...
   For example, you can [resize your virtual machine](https://learn.microsoft.com/en-us/azure/virtual-machines/resize-vm).
...
1. On the **Overview** page, select **Run history**. To view more details about a workflow run, select that run.
   ...
   ![Screenshot shows the logic app overview page with a successful run selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/logic-app-run-history.png)
...
1. To view the inputs and outputs for each operation, select that operation.
   ...
   ![Screenshot shows the workflow run history with details about each run.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/logic-app-run-history-details.png)
...
- To stop running your workflow without deleting your work, disable your app. On the logic app sidebar, select **Overview**. On the toolbar, select **Disable**.
  ...
  ![Screenshot shows logic app Overview page with Disable button selected.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/event-grid/media/monitor-virtual-machine-changes-logic-app/turn-off-disable-logic-app.png)
...
- [Route custom events to web endpoint with Azure CLI and Event Grid](https://learn.microsoft.com/en-us/azure/event-grid/custom-event-quickstart)
...
- [Azure Event Grid samples for .NET](https://learn.microsoft.com/en-us/samples/azure/azure-sdk-for-net/azure-event-grid-sdk-samples/)
- [Azure Event Grid samples for Java](https://learn.microsoft.com/en-us/samples/azure/azure-sdk-for-java/eventgrid-samples/)
- [Azure Event Grid samples for Python](https://learn.microsoft.com/en-us/samples/azure/azure-sdk-for-python/eventgrid-samples/)
- [Azure Event Grid samples for JavaScript](https://learn.microsoft.com/en-us/samples/azure/azure-sdk-for-js/eventgrid-javascript/)
- [Azure Event Grid samples for TypeScript](https://learn.microsoft.com/en-us/samples/azure/azure-sdk-for-js/eventgrid-typescript/)
