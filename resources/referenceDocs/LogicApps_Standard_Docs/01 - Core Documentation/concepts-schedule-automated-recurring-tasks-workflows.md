<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/concepts-schedule-automated-recurring-tasks-workflows -->
...
You can schedule and run recurring workloads without creating a separate Consumption logic app for each scheduled job and running into the [limit on workflows per region and subscription](https://learn.microsoft.com/en-us/azure/logic-apps/logic-apps-limits-and-config#definition-limits). Instead, you can either [create a Standard logic app with multiple workflows](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-azure-portal), or use the Consumption logic app pattern that's created by the [Azure Quickstart Template: Logic Apps job scheduler](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.logic/logicapps-jobscheduler/).
...
You can start your logic app workflow by using the [Recurrence trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence) or [Sliding Window trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-sliding-window), which isn't associated with any specific service or system. These triggers start and run your workflow based on your specified recurrence where you select the interval and frequency, such as the number of seconds, minutes, hours, days, weeks, or months. You can also set the start date and time along with the time zone. Each time that a trigger fires, Azure Logic Apps creates and runs a new workflow instance for your logic app.
...
* **Recurrence**: Runs your workflow at regular time intervals based on your specified schedule. If the trigger misses recurrences, for example, due to disruptions or disabled workflows, the **Recurrence** trigger doesn't process the missed recurrences but restarts recurrences with the next scheduled interval.
  ...
  >
  > * Use the [Sliding Window trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-sliding-window), rather than the **Recurrence** trigger.

  For more information, see [Create, schedule, and run recurring tasks and workflows with the Recurrence trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence).
...
* **Sliding Window**: Runs your workflow at regular time intervals that handle data in continuous chunks. If the trigger misses recurrences, for example, due to disruptions or disabled workflows, the Sliding Window trigger goes back and processes the missed recurrences.
  ...
  You can specify a start date and time, time zone, and a duration to delay each recurrence in your workflow. This trigger doesn't support advanced schedules, for example, specific hours of the day, minutes of the hour, and days of the week. For more information, see [Create, schedule, and run recurring tasks and workflows with the Sliding Window trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-sliding-window).
...
* **Delay**: Wait to run the next action for the specified number of time units, such as seconds, minutes, hours, days, weeks, or months. For more information, see [Delay the next action in workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-delay).

* **Delay until**: Wait to run the next action until the specified date and time. For more information, see [Delay the next action in workflows](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-delay).
...
Recurring built-in triggers, such as the [Recurrence trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence), run directly and natively on the Azure Logic Apps runtime. These triggers differ from recurring connection-based managed connector triggers where you need to create a connection first, such as the Office 365 Outlook managed connector trigger.
...
* [Troubleshoot recurrence issues](https://learn.microsoft.com/en-us/azure/logic-apps/concepts-schedule-automated-recurring-tasks-workflows.md#recurrence-issues)
...
To run the **Recurrence** trigger only once on the last day of the month, see [Run one time on the last day of the month](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence.md#run-only-last-day-of-month).
...
* Consider using a [**Sliding Window** trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-sliding-window) instead of a **Recurrence** trigger to avoid missed recurrences.

## Next steps

* [Create, schedule, and run recurring tasks and workflows with the Recurrence trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-recurrence)
* [Create, schedule, and run recurring tasks and workflows with the Sliding Window trigger](https://learn.microsoft.com/en-us/azure/connectors/connectors-native-sliding-window)
