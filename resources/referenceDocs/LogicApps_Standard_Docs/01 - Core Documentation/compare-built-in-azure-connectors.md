<!-- Source: https://learn.microsoft.com/en-us/azure/connectors/compare-built-in-azure-connectors -->
...
For Standard logic app resources, the workflow designer shows the available connectors and operations using the labels named **Built-in** and **Shared**. The **Built-in** label applies to [*built-in* operations](https://learn.microsoft.com/en-us/azure/connectors/built-in), which natively run in the same cluster and runtime as your Standard logic app in single-tenant Azure Logic Apps. This label also applies to connectors known as *service providers*, which are actually custom extensions that are implemented based on Azure Functions. Anyone can create their own service provider connector.

The **Shared** label applies to [connectors hosted in Azure and *managed* by Microsoft](https://learn.microsoft.com/en-us/azure/connectors/managed) that run in *shared connector clusters* in the multitenant Azure cloud. These shared managed connector clusters exist separately from the single-tenant Azure Logic Apps engine and runtime, which run in a different cluster. If your workflow invokes a shared connector operation, Azure Logic Apps makes a call to the connector in the managed connector clusters. In turn, the connector might then call the backend target service, which can be Office 365, Salesforce, and so on.
...
For a shared managed connector to work, your backend service, such as Office 365 or SQL Server, must allow traffic through the [outbound IP addresses for managed connectors](https://learn.microsoft.com/en-us/connectors/common/outbound-ip-addresses) in the region where you created your logic app resource.
