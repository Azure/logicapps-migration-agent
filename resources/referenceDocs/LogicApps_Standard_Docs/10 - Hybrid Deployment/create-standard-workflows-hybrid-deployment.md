<!-- Source: https://learn.microsoft.com/en-us/azure/logic-apps/create-standard-workflows-hybrid-deployment -->
...
For an architectural overview that shows where Standard logic app workflows are hosted and run in a partially connected environment, see [Set up infrastructure requirements for hybrid deployment for Standard logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements).
...
- The following on-premises resources, which must all exist within the same network for the required connectivity:
  ...
  To meet these requirements, [set up these on-premises resources to support hybrid deployment for Standard logic apps](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements).

- To work in Visual Studio Code, you need the Azure Logic Apps (Standard) extension for Visual Studio Code with the [related prerequisites](https://learn.microsoft.com/en-us/azure/logic-apps/create-single-tenant-workflows-visual-studio-code#prerequisites).
...
1. On the **Create Logic App (Hybrid)** page, provide the following information:
   ...
   | **Resource Group** | Yes | <*Azure-resource-group-name*> | The [Azure resource group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/overview.md#terminology) where you create your hybrid app and related resources. This name must be unique across regions and can contain only letters, numbers, hyphens (**-**), underscores (**_**), parentheses (**()**), and periods (**.**). <br><br>This example creates a resource group named **Hybrid-RG**. |
   ...
   | **Region** | Yes | <*Azure-region*> | An Azure region that is [supported for Azure Container Apps on Azure Arc-enabled AKS](https://learn.microsoft.com/en-us/azure/container-apps/azure-arc-overview.md#limitations). <br><br>This example uses **East US**. |
   ...
   ![Screenshot shows Azure portal and logic app creation page.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-standard-workflows-hybrid-deployment/create-logic-app-hybrid-portal.png)
...
1. On the **Storage** page, provide the following information about the storage provider and SMB file share that you previously set up:
   ...
   | Property | Required | Value | Description |
   |----------|----------|-------|-------------|
   | **SQL connection string** | Yes | <*sql-server-connection-string*> | The SQL Server connection string that you previously saved. For more information, see [Create SQL Server storage provider](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements#create-storage-provider). |
...
1. After Azure completes deployment, select **Go to resource**.
   ...
   ![Screenshot shows Azure portal with Standard logic app for hybrid deployment created as a container app.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-standard-workflows-hybrid-deployment/logic-app-hybrid-portal.png)
...
1. After the designer opens, build your workflow by adding a trigger and actions.
   ...
   For more information, see [Build a workflow with a trigger and actions](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action).
...
1. From the location list, select an Azure region that is [supported for Azure Container Apps on Azure Arc-enabled AKS](https://learn.microsoft.com/en-us/azure/container-apps/azure-arc-overview.md#limitations).
...
1. Build your workflow as usual by adding a trigger and actions. For more information, see [Build a workflow with a trigger and actions](https://learn.microsoft.com/en-us/azure/logic-apps/create-workflow-with-trigger-or-action).
...
1. To use zip deployment with an existing Standard logic app resource in the Azure portal, you must manually add the app registration as an app setting in your Standard logic app resource.
   ...
   For more information, see [Add app registration values to your Standard logic app](https://learn.microsoft.com/en-us/azure/logic-apps/create-standard-workflows-hybrid-deployment#add-app-registration-values-to-your-standard-logic-app&tabs=azure-portal).
...
A Standard logic app with the hybrid hosting option automatically creates a new *revision*, which is a [versioning concept from Azure Container Apps](https://learn.microsoft.com/en-us/azure/container-apps/revisions), whenever you save changes to a child workflow. This revision might take a little time to activate, which means that after you save any changes, you might want to wait several 
...
For partially connected and on-premises scenarios, you can set up your Standard logic app to emit telemetry based on the [OpenTelemetry-supported](https://opentelemetry.io/) app settings that you define for the specific environment. By default, this telemetry data is sent to Application Insights. For more information, see [Enable enhanced telemetry in Application Insights for Standard workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/enable-enhanced-telemetry-standard-workflows).

<a name="change-vcpu-memory"></a>

## Change vCPU and memory allocation in the Azure portal

You can edit the vCPU and memory settings for your Standard logic app resource. These changes affect the [billing charge](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements#billing) for your Standard logic app workloads.
...
1. Based on your scenario, configure the remaining options.
   ...
   - [Configure ingress for your app in Azure Container Apps](https://learn.microsoft.com/en-us/azure/container-apps/ingress-how-to.md?pivots=azure-portal)
   - [Set up IP ingress restrictions in Azure Container Apps](https://learn.microsoft.com/en-us/azure/container-apps/ip-restrictions.md?pivots=azure-portal)
...
1. For the object ID, follow these steps:
   ...
   1. On the **Overview** page, select **Managed application in local directory** link for your app registration as shown:
      ...
      ![Screenshot shows app registration with selected link for managed application in local directory.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-standard-workflows-hybrid-deployment/managed-application-link.png)

   1. On the page that opens, copy and save the **Object ID** value:

      ![Screenshot shows app registration with selected object ID.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-standard-workflows-hybrid-deployment/app-registration-object-id.png)
...
1. To create the app registration, use the [**az ad sp create** command](https://learn.microsoft.com/en-us/cli/azure/ad/sp#az-ad-sp-create).

1. To review all the properties, use the [**az ad sp show** command](https://learn.microsoft.com/en-us/cli/azure/ad/sp#az-ad-sp-show).
...
1. On the resource menu, under **Settings**, select **Containers**, and then select the **Environment variables** tab.
   ...
   For more information about app settings and host settings, see [Edit app settings and host settings](https://learn.microsoft.com/en-us/azure/logic-apps/edit-app-settings-host-settings).
...
1. On the **Overview** page, next to the **Application Url** field, select the resource URL.
   ...
   ![Screenshot shows browser and logic app running as a website.](https://raw.githubusercontent.com/MicrosoftDocs/azure-docs/main/articles/logic-apps/media/create-standard-workflows-hybrid-deployment/running-logic-app-hybrid-deployment.png)
...
- [Create node pools for a cluster in Azure Kubernetes Service (AKS)](https://learn.microsoft.com/en-us/azure/aks/create-node-pools)
- [Manage node pools for a cluster in Azure Kubernetes Service (AKS)](https://learn.microsoft.com/en-us/azure/aks/manage-node-pools)
...
For more information, see [Container Storage Interface (CSI) drivers on Azure Kubernetes Service (AKS)](https://learn.microsoft.com/en-us/azure/aks/csi-storage-drivers).

## Related content

- [Set up requirements for Standard logic app deployment on your own infrastructure](https://learn.microsoft.com/en-us/azure/logic-apps/set-up-standard-workflows-hybrid-deployment-requirements)
- [Enable enhanced telemetry in Application Insights for Standard workflows in Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/enable-enhanced-telemetry-standard-workflows)